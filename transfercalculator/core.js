...
function init() { // Run startup sequence
...
function startSimulation() { // Start system update funciton timer
...
function generateOrbitalData() { // Import the data from firebase and finish startup
	setTimeout(function () {
        // Log the course of action
        console.log("Firebase unavailable. Generating now: " + round((new Date().getTime() - now.getTime()) / 1000, 2) + "s")
...
function finishStartUp() { // Bring it to full functionality - if everything is loaded

	// Make sure both the parameters are present AND the ship is loaded
	if (Object.keys(orbitalTimes).length !== 0 && shipSurfaceMesh) {

		setTimeout(function () { // Delay slightly to let it load fully
			...
			console.log("SYSTEM ONLINE: " + round((new Date().getTime() - now.getTime()) / 1000, 2) + "s")
		...
		console.log("ORBITAL DATA INITIALISED: " + round((new Date().getTime() - now.getTime()) / 1000, 2) + "s")
	}
	else if (shipSurfaceMesh) {
		// Log the ship mesh loading time
		console.log("SHIP MESH INITIALISED: " + round((new Date().getTime() - now.getTime()) / 1000, 2) + "s")
	}
}
...
function startPostLoading() {
	...
	document.getElementById("soundDiv").innerHTML = '<audio id="KSP" src="music/KSP.mp3" autostart="false" currentTime="0"></audio><audio id="ASZ" src="music/ASZ.mp3" autostart="false" currentTime="20"></audio>'
...
function updateFirebaseData(fireCheck) {
	...
	if ((checkSec1 % 673) + (checkSec1 % 2) + (checkSec1 % 9) + (checkSec1 % 61) == 4 && checkSec2 == "CA" && window.location.href.indexOf("orbital_mechanics") > -1 && firebase) {
		...
		for (var name in originalPlanets) {
			...
			for (var key in originalPlanets[name]) {
				if (key != "mapClass" && key != "viewingClass" && key != "axialTilt") {
...
function startPorkchopListener() { // Start listener on the porkchop plot - for logging courses
	document.getElementById('porkchop').addEventListener('click', function (evt) {
		logCourse(evt.clientX, evt.clientY);
	}, false);
}

function startEasterListener() { // Start listener for easter egg triggers
	...
	document.addEventListener('keydown', function (e) {
		...
		if (key == requiredKey) {
			...
			if (konamiCodePosition == konamiCode.length && eggEnabled == false) {
				// Run easter egg systems
				swal("You found the Easter Egg!", "This is actually an intented Easter Egg, not like Konami...", "success");
				egg();
		...
		if (key == requiredKeyKerbal) {
			...
			// Upon the last key, activate Easter Eggs (if not already activated)
			if (kerbalCodePosition == kerbalCode.length && eggEnabled == false) {

				// Run easter egg systems
				swal("You found the Easter Egg!", "Unfortunately, there is no Kerbol system in this.", "success");
				playSound("KSP");
				egg();
		...
		if (key == starKeyKerbal) {
			...
			if (starCodePosition == starCode.length && eggEnabled == false) {
				...
				// Run easter egg systems
				swal("You found the Easter Egg!", "His Tesla Roadster is under minor bodies, still blasting Life on Mars", "success");
				egg();
...
function startTooltip() { // Start the tooltip detector

	// Start the Threejs mouse move projector
	projector = new THREE.Projector();
	document.addEventListener('mousemove', onDocumentMouseMove, false);
	document.addEventListener('mousedown', onDocumentMouseDown, false);
	document.getElementById('shipToolTipContent').addEventListener('mousedown', onTooltipClick, false);
}

function onPointerRestricted() { // Run this function that's essential for VR
...
function onPointerUnrestricted() { // Run this function that's essential for VR
...
function executeURLInstr() { // Execute instructions contained in the URL for examples

	// Save the URL for parsing
	var URL = window.location.href;

	if (URL.indexOf("?e=") > -1) { // Check there is anything to parse

		// General "look at this planet" section
		for (var name in planets) {
			if (URL.lastIndexOf(name) > URL.indexOf("?e=") && name != "sun") { // CHeck all planets (except the sun)
				...
				var size = 6;
				var cameraVec = addVec(setMagnitude(currentPositions[name], -1 * planets[name]["r"] * size), [0, 0, 1 * planets[name]["r"] * size / 2])

				if (URL.lastIndexOf("Termin") > URL.indexOf("?e=")) { // If looking at the terminator, move the camera to look at it from the side
					cameraVec = addVec(multiplyVec(1 / 2, [-cameraVec[1], cameraVec[0], cameraVec[2]]), multiplyVec(1 / 20, cameraVec))
					document.getElementById("inputTimeScale").value = 3;
					changeTimeScale();
				}

				// Set camera position
				camera.position.copy(threeVector(multiplyVec(totalScale, cameraVec)))
			}
		}

		// Specific scripts
		if (URL.indexOf("egg") > URL.indexOf("?e=")) { // If eggs should be turned on, turn them on
		...
		if (URL.indexOf("tour") > URL.indexOf("?e=")) { // Start the tour of the Solar System

			console.log("Starting Solar System Tour");

			startSystemTour();
		}
		if (URL.indexOf("jupiterShadow") > URL.indexOf("?e=")) { // Look at the shadow of Io on Jupiter from Io
			...
			camera.position.copy(threeVector(multiplyVec(totalScale, [0.00007027538734538652, 0.00013350090234620637, -0.0000038122306606214185])))
		}
		if (URL.indexOf("saturnShadow") > URL.indexOf("?e=")) { // Showing off the shadow of the rings of Saturn
			...
			// Set the camera position - predetermined to show this
			camera.position.copy(threeVector(multiplyVec(totalScale, [0.002107537387441072, 0.0013708102883818185, 0.0005713832910721611])))
		}
		if (URL.indexOf("EMTrans") > URL.indexOf("?e=")) { // Looking at an Earth-Mars Transfer
		...
		if (URL.indexOf("IGTrans") > URL.indexOf("?e=")) {
			...
			camera.position.copy(threeVector(multiplyVec(totalScale, [0.007310935589102119, 0.002088179982759897, 0.007727425436799634])))
		...
		if (URL.indexOf("ISTrans") > URL.indexOf("?e=")) {
			...
			var tempTimer = setInterval(function () { // Set the camera to look at the ship - after a delay to make sure it parses right
				if (magnitude(lastShipLocation) != 0) {
					camera.position.copy(threeVector(multiplyVec(totalScale, addVec(lastShipLocation, [0.0001, 0.0001, 0]))))
		...
		if (URL.indexOf("EpsteinTrans") > URL.indexOf("?e=")) { // Demonstrate and Epstein transfer
			...
			var tempTimer = setInterval(function () { // Set the camera position
				if (EpsteinArray) {
					camera.position.copy(threeVector(multiplyVec(totalScale, addVec(EpsteinArray, [0.0000015, 0.0000015, 1]))))
					clearInterval(tempTimer);
				}
			}, 100);

		}
		if (URL.indexOf("propDelay") > URL.indexOf("?e=")) { // Look at the propagation delay
			...
			// Set camera position
			camera.position.copy(threeVector(multiplyVec(totalScale, [0, 0, 3])))
		}
		if (URL.indexOf("propDelayJupiter") > URL.indexOf("?e=")) { // Look at the propagation delay
			...
			// Set camera position
			camera.position.copy(threeVector(multiplyVec(totalScale, [0, 0, 0.01])))
		}
	}
}

function sortPlanetData(first) { // Sort planets array
	// Sorting rules
	...
	planetKeys.sort(function (x, y) {
		...
		if (centerX == "sun" && centerY != "sun") { // Prioritise planet over moon (X)
			return -1;
		}
		else if (centerX != "sun" && centerY == "sun") { // Prioritise planet over moon (Y)
			return 1;
		}
		else if (centerX == "sun" && centerY == "sun") { // If both around the sun, sort by semi-major axis
			return aX - aY;
		}
		else { // IF BOTH MOONS
			if (centerX == centerY) { // Sort by semi-major axis if same center
				return aX - aY;
			}
			else { // Otherwise, sort by semi-major axis of center
...
function correctPlanetParams() { // Run calculations for planetary parameters such as Sphere of Influence

	for (var planet in planets) { // Iterate through all planets

		// If no gravParam specified, calculate from the mass
		if (!planets[planet]["gravParam"]) {
			planets[planet]["gravParam"] = planets[planet]["mass"] * gravitationalConstant;
		}
		...
		planets[planet]["SOI"] = planA * Math.pow(planParam / (3 * centParam), 1 / 3);
		...
		if (planets[planet]["center"] != "sun") {
			if (planets[planets[planet]["center"]]["axialTilt"]) {
				planets[planet]["axialTilt"] = planets[planets[planet]["center"]]["axialTilt"];
			}
		}
...
function managePhone() { // Deal with users trying to use a phone

	// Test to see if the user is on a phone
	userOnPhone = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
...
function runLoadingText() { // Update the boot screen messages

	// Calculate the number of seconds (inside a 4s interval)
	var seconds = ((new Date().getTime() - lastLoadingTime.getTime()) / 1000) % 4;

	if (seconds < 1 || (new Date().getTime() - lastLoadingTime.getTime()) > 4000) { // Update the message after 4 seconds
		if (messageUpdated == false) {
			message = bootMessages[Math.floor(Math.random() * loadingMessages.length)];
			messageUpdated = true;
	...
	document.getElementById("bootText").innerHTML = message + ".".repeat(Math.floor(seconds));
	if (stereoDisp) {
		document.getElementById("bootTextAux").innerHTML = message + ".".repeat(Math.floor(seconds));
	}
}

function startTimeDatePickers() { // Initialise time and date pickers - to see what all the init data means, go look at the initial webpage
	$('#timepicker').timepicker({ // Start the timepicker
		timeFormat: 'HH:mm',
		interval: 60,
		minTime: '0',
		maxTime: '11:59pm',
	...
	$('#datepicker').datepicker({ // Start the datepicker
		autoClose: true
	})
}

function startStarInterface() { // Update which stars are visible in the star dropdown
	...
	for (var name in stars) {
		printer += "<option value='" + stars[name]["distance"] + "'>" + name + "</option>";
	}

	// Push printer to dropdown and select the first option
	document.getElementById('starTarget').innerHTML = printer;
	document.getElementById('starTarget').value = stars[Object.keys(stars)[0]]["distance"];
	document.getElementById('ISDist').value = stars[Object.keys(stars)[0]]["distance"];
}

function correctPixelSizes() { // If it's not rendering at the true scale, fix the pixel sizes for UI consistency
	pixelRatio = renderer.context.canvas.width / WIDTH
}

// Graphics Initialisation

function startThreeJS() { // Initialise all ThreeJS systems, such as the scene, lights and cameras

	// Start camera and move it to the position
	camera = new THREE.PerspectiveCamera(70, WIDTH / HEIGHT, 2 * totalScale / Math.pow(10, 8), 2 * starFieldDist * totalScale);
	initialCamera = [0, -0.0001, 2 * totalScale / Math.tan(DtoR(camera.fov / 2))];
	...
	if (stereoDisp) {
		...
		// Init both cameras
		cameraLeft = new THREE.PerspectiveCamera(camera.fov / 2, WIDTH / HEIGHT, camera.near, camera.far);
		cameraRight = new THREE.PerspectiveCamera(camera.fov / 2, WIDTH / HEIGHT, camera.near, camera.far);
	...
	spotLight.shadow.camera = new THREE.OrthographicCamera(-size, size, size, -size, 0.01 * totalScale, 1 * totalScale);
...
function createStarField() {
	...
	var geometry = new THREE.SphereGeometry(starFieldDist * totalScale, geoRes, geoRes); // 1000000 AU out is close enough to infinity for this
...
function showPlanetaryMarkers() { // Control which planetary markers are shown

	// Wait until a small bit after startup to make sure it's all there
	setTimeout(function () {

		for (var name in planets) { // Iterate through all planets

			if (name != "sun") { // Don't do the sun because it's special
...
function createStarMarkers() { // Create markers at the starfield that allow for a tooltip to mark the star
	...
	for (var name in stars) {
		...
		var RA = (180 * star["RA"][0] / 12) + (180 * star["RA"][1] / (12 * 60)) + (180 * star["RA"][2] / (12 * 60 * 60));
		var DE = (180 * star["DE"][0] / 180) + (180 * star["DE"][1] / (180 * 60)) + (180 * star["DE"][2] / (180 * 60 * 60));
		...
		// Set the marker data
		var position = setMagnitude(starVector, starFieldDist * totalScale * 0.9)
...
function createShipSurface(type) { // Establish the surface of the ship - different types are kept in storage

	if (type == "sketch" || lowRes) { // This is the default and boring two cones and a cylinder - same as the marker
		...
		// Create mesh
		shipSurfaceMesh = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({
			color: shipColour,
			side: THREE.DoubleSide,
			opacity: 0,
			transparent: true,
		...
		// Finish loading in
		console.log("Sketch loaded at: " + round((new Date().getTime() - now.getTime()) / 1000, 2) + "s")
		shipLoading = false;
		finishStartUp();

	}
	else if (type == "APOLLOCSM") { // This is the Apollo Command Service Module - textured flat grey
		...
		setTimeout(function () {
			loader.load('models/special/APOLLOCSM.obj', function (object) {
				...
				// Create material
				var settings = { // Make it shiny, and grey
					color: 0x888a7c
				}
				var material;
				...
				for (var index in object.children) {
					setTimeout(function (index) {
						...
						var newGeometry = new THREE.Geometry().fromBufferGeometry(child.geometry);
				...
				setTimeout(function () { // Run this after it's finished with parsing the geometry
	...
	else if (type == "MCRNTACHI" || type == "MCRNTACHI HD") { // This is the Rocinante (as the MCRN Tachi), from The Expanse. If you've seen it, you know this. Otherwise, you should watch The Expanse!
		...
		loader.load(baseURL + 'models/glTF/' + type + '/scene.gltf', function (loadedScene) {
			...
			setTimeout(function () {
				...
				shipSurfaceMesh.traverse(function (child) { // Go through all the children and apply shadow rules
				...
				// Finish loading in
				console.log(stringAdd + "Roci Loaded. Good luck, and Godspeed: " + round((new Date().getTime() - now.getTime()) / 1000, 2) + "s");
...
function fixPlanetRotation(name) { // Start up the planet rotation and align axes

	if (name != "sun" && name != "ship") { // Don't do anything if it is the sun or the ship, they are special

		if (planets[name]["axialTilt"]) { // If the planet has an axial tilt, rotate it to align it to spin around that
			...
			var alignVec = threeVector(planets[name]["axialTilt"]).clone().normalize();

			// Align the planet
			planets[name]["surfaceMesh"].quaternion.setFromUnitVectors(axis, alignVec);

			// If it has an atmosphere, align that too
			if (planets[name]["atmoMesh"]) { // Fix the atmosphere as well
				planets[name]["atmoMesh"].quaternion.setFromUnitVectors(axis, alignVec);
			}
		}
		if (planets[name]["rotEpoch"]) { // Rotate planets by their rotational epoch to start off
			...
			var rotation = (2 * Math.PI * percent + planets[name]["rotEpoch"]) % (2 * Math.PI);
...
function executePageType() { // Figure out the page type and modify variables accordingly

	var URL = window.location.href;

	if (URL.indexOf("3DSBS.html") > -1) { // Resolve URL systems - use this to determine mode
...
function createGuides() { // Create Vernal equinox line
	...
	geometry.vertices.push(new THREE.Vector3(0 * totalScale, 0, 0),
...
function createPlanets() { // Create all planetary surfaces

	for (var name in planets) {
		if (name != "sun") { // The sun is special, so don't do it
...
function drawOrbits() { // Draw orbital lines for each planet
...
function createOrbit(name, startDegree, endDegree) { // Create the ThreeJS geometry for the orbit track
...
function renderEarthNight() { // Render the night side of Earth
	...
	// Match the alignment vector to the axial tilt
	var alignVec = threeVector(planets["earth"]["axialTilt"]).clone().normalize();
	...
	// Rotate by the needed amount, in a similar way to planetary epoch
	var rotation = (2 * Math.PI * percent + planets["earth"]["rotEpoch"]) % (2 * Math.PI);
	nightEarth.rotateOnAxis(axis, rotation);
}

function renderPlanet(name) { // Create the planetary surface and marker
	...
	if (name == "tycho station") { // This is a complex and manually generated geometry - kept separate for ease of programming
	...
	if (name == "sun") { // Create a special surface mesh for the sun

		material = new THREE.MeshBasicMaterial({ // This is for the sun
			color: white,
	...
	if (name != "sun") { // The sun's is different 

		// Initialise the flare
		var flareColor = new THREE.Color(colour);
		var textureFlare;
		if (stereoDisp) {
			textureFlare = textureLoader.load("assets/flares/planetFlareSBS.png");
		}
		else {
		...
		if (renderingLensFlares) {
			...
			if (webVR && false) {
				...
				lensFlare.addElement(new THREE.LensflareElement(textureFlare, 1, 0.0, flareColor, THREE.AdditiveBlending))
		...
		if (!lowRes) {
			if (stereoDisp) {
				textureFlare = textureLoader.load("assets/flares/lensflareSBS.png");
			}
			else {
				textureFlare = textureLoader.load("assets/flares/lensflare.png");
			}
		}
		else {
			textureFlare = textureLoader.load("assets/flares/planetFlare.png");
		...
		if (webVR && false) {
			...
			sunFlare.addElement(new THREE.LensflareElement(textureFlare, 1, 0.0, flareColor, THREE.AdditiveBlending));
...
function theRingMesh() {
	...
	var ringGeo = new THREE.TorusGeometry(r * 49 / 50 * totalScale, (r / 50) * totalScale, geoRes, geoRes)
	var ringCoreGeo = new THREE.TorusGeometry(r * 49 / 50 * totalScale, (r / 200) * totalScale, geoRes, geoRes);
	var hideGeo = new THREE.RingGeometry(0.000001, r * 48.2 / 50 * totalScale, geoRes);
	...
	ringMesh.traverse(function (child) { // Go through all the children and apply shadow rules
...
function tychoMesh() { // Generate the mesh for Tycho Station and the Nauvoo
	var name = "tycho station"

	// Create geometries
	var tychoGeo = new THREE.SphereGeometry(250 * convertDistance("M", "AU") * totalScale, geoRes, geoRes);

	var nauvooGeo = new THREE.CylinderGeometry(250 * convertDistance("M", "AU") * totalScale, 250 * convertDistance("M", "AU") * totalScale, 1500 * convertDistance("M", "AU") * totalScale, geoRes);
	var nauvooCommGeo = new THREE.CylinderGeometry(62.5 * convertDistance("M", "AU") * totalScale, 62.5 * convertDistance("M", "AU") * totalScale, 100 * convertDistance("M", "AU") * totalScale, geoRes);
	var nauvooSpikeGeo = new THREE.CylinderGeometry(0 * convertDistance("M", "AU") * totalScale, 10 * convertDistance("M", "AU") * totalScale, 100 * convertDistance("M", "AU") * totalScale, geoRes);
	var nauvooDocksGeo = new THREE.TorusGeometry(200 * convertDistance("M", "AU") * totalScale, 5 * convertDistance("M", "AU") * totalScale, geoRes, geoRes)
	var nauvooReactorGeo = new THREE.SphereGeometry(50 * convertDistance("M", "AU") * totalScale, geoRes, geoRes);
	var nauvooConnectGeo = new THREE.CylinderGeometry(10 * convertDistance("M", "AU") * totalScale, 10 * convertDistance("M", "AU") * totalScale, 250 * convertDistance("M", "AU") * totalScale, geoRes);

	var nauvooDriveGeo = new THREE.CylinderGeometry(25 * convertDistance("M", "AU") * totalScale, 25 * convertDistance("M", "AU") * totalScale, 100 * convertDistance("M", "AU") * totalScale, geoRes);
	var nauvooDriveConnectGeo = new THREE.CylinderGeometry(5 * convertDistance("M", "AU") * totalScale, 5 * convertDistance("M", "AU") * totalScale, 350 * convertDistance("M", "AU") * totalScale, geoRes);
	var nauvooHabGeo = new THREE.CylinderGeometry(270 * convertDistance("M", "AU") * totalScale, 270 * convertDistance("M", "AU") * totalScale, 350 * convertDistance("M", "AU") * totalScale, geoRes);

	var nauvooLiftGeo = new THREE.CylinderGeometry(25 * convertDistance("M", "AU") * totalScale, 25 * convertDistance("M", "AU") * totalScale, 1600 * convertDistance("M", "AU") * totalScale, geoRes);
	var nauvooDocksConnectGeo = new THREE.CylinderGeometry(5 * convertDistance("M", "AU") * totalScale, 5 * convertDistance("M", "AU") * totalScale, 400 * convertDistance("M", "AU") * totalScale, geoRes);

	var tychoRingGeo = new THREE.CylinderGeometry(350 * convertDistance("M", "AU") * totalScale, 350 * convertDistance("M", "AU") * totalScale, 50 * convertDistance("M", "AU") * totalScale, geoRes);
	var antennaGeo = new THREE.CylinderGeometry(0 * convertDistance("M", "AU") * totalScale, 25 * convertDistance("M", "AU") * totalScale, 500 * convertDistance("M", "AU") * totalScale, geoRes);
	var connectGeo = new THREE.CylinderGeometry(25 * convertDistance("M", "AU") * totalScale, 25 * convertDistance("M", "AU") * totalScale, 400 * convertDistance("M", "AU") * totalScale, geoRes);
	...
	for (var i = 0; i < 4; i++) {
		nauvooDrivesConnect.push(new THREE.Mesh(nauvooDriveConnectGeo, nauvooMaterial))
	}
	for (var i = 0; i < 4; i++) {
		nauvooDocksConnect.push(new THREE.Mesh(nauvooDocksConnectGeo, nauvooMaterial))
	}
	for (var i = 0; i < 2; i++) {
	...
	subGroup.rotation.set(5.847131682738216, 3.0667165806460956, 3.829487887805754) // Randomly selected
	...
	nauvooCommMesh.position.x = -(800 + 50) * convertDistance("M", "AU") * totalScale

	nauvooSpikeMesh.rotation.z = Math.PI / 2;
	nauvooSpikeMesh.position.x = -(800 + 50 + 100) * convertDistance("M", "AU") * totalScale

	nauvooDocksMesh.rotation.y = Math.PI / 2;
	nauvooDocksMesh.scale.z = 5;
	nauvooDocksMesh.position.x = (700 + 80) * convertDistance("M", "AU") * totalScale

	nauvooReactorMesh.rotation.z = Math.PI / 2;
	nauvooReactorMesh.position.x = (700 + 200 + 50) * convertDistance("M", "AU") * totalScale

	nauvooConnectMesh.rotation.z = Math.PI / 2;
	nauvooConnectMesh.position.x = (700 + 125) * convertDistance("M", "AU") * totalScale
	...
	for (var i = 0; i < 8; i++) {
		...
		nauvooDrives[i].position.y = (yBias) * convertDistance("M", "AU") * totalScale
		nauvooDrives[i].position.x = (700 + 200 + 50) * convertDistance("M", "AU") * totalScale
		nauvooDrives[i].position.z = -(zBias) * convertDistance("M", "AU") * totalScale
	}
	...
	for (var i = 0; i < 4; i++) {
		...
		nauvooDrivesConnect[i].position.x = (700 + 200 + 50) * convertDistance("M", "AU") * totalScale
	}
	...
	for (var i = 0; i < 2; i++) {
		...
		nauvooDocksConnect[i].position.x = (700 + 80) * convertDistance("M", "AU") * totalScale
	}

	for (var i = 0; i < 2; i++) {

		nauvooHabs[i].rotation.z = Math.PI / 2;
		nauvooHabs[i].position.x = ((i * (150 + 350)) - 500) * convertDistance("M", "AU") * totalScale
	}
	...
	for (var i = 0; i < 2; i++) {
		...
		nauvooLifts[i].position.y = (yBias) * convertDistance("M", "AU") * totalScale
		nauvooLifts[i].position.x = (100) * convertDistance("M", "AU") * totalScale
		nauvooLifts[i].position.z = -(zBias) * convertDistance("M", "AU") * totalScale
	}
	...
	nauvooGroup.position.y = -(250 + 400) * convertDistance("M", "AU") * totalScale
	...
	subGroup.traverse(function (child) { // Go through all the children and apply shadow rules
		if (child instanceof THREE.Mesh || child instanceof THREE.Group) {
...
function placeSphere(name, mesh) { // Place the sphere in the planets data and in the scene
...
function addAtmo(name) { // Add the atmosphere for the planet if needed

	// Find data and initialise variables
	var r = planets[name]["r"];
	var atmoMesh;

	if (name == "earth") { // If the planet is Earth, add Earth's atmosphere
	...
	if (name == "titan") { // If the planet is Titan, add Titan's atmosphere
	...
	if (name == "venus") { // If the planet is Venus, add Venus's atmosphere
	...
	if (name == "mars") { // If the planet is Mars, add mars's atmosphere
		...
		marsOpacity = Math.random() * (marsOpacityUpper - marsOpacityLower) + marsOpacityLower;
...
function onDocumentMouseMove(event) { // When the mouse is moved, update the mouse position
	...
	if (intersectedObject) {
		if (intersectedObject.name == "ship" || intersectedObject.name == "rocinante") {
...
function moveTooltip(position, id) { // Move the selected tooltip to the correct specified position

	var sizeReduction = (document.getElementById(id + "Content").clientWidth / 4)

	// Decide on the X position
	if ((position.x < window.innerWidth / 2 - toolTipMargin) || (document.getElementById(id).style.left != "" && position.x < window.innerWidth / 2 + toolTipMargin)) { // Mouse in left half of screen (or over the sun - to keep consistent)

		document.getElementById(id).style.left = position.x - toolTipCanvasMargin + "px";
		if (stereoDisp) {
			document.getElementById(id).style.left = position.x - toolTipCanvasMargin - sizeReduction + "px";
		}
		document.getElementById(id).style.right = "";
	}
	else { // Mouse in right half of screen (or over the sun - to keep consistent)

		document.getElementById(id).style.left = "";
		document.getElementById(id).style.right = (window.innerWidth - position.x - toolTipCanvasMargin) + "px";
		if (stereoDisp) {
			document.getElementById(id).style.right = (window.innerWidth - position.x - toolTipCanvasMargin + sizeReduction) + "px";
		}
	}

	// Decide on the Y position
	if ((position.y > window.innerHeight / 2 - toolTipMargin) || (document.getElementById(id).style.bottom != "" && position.y > window.innerHeight / 2 + toolTipMargin)) { // Mouse in bottom half of screen (or over the sun - to keep consistent)

		document.getElementById(id).style.top = "";
		document.getElementById(id).style.bottom = (window.innerHeight - position.y - toolTipCanvasMargin) + "px";
	}
	else { // Mouse in top half of screen (or over the sun - to keep consistent)

		document.getElementById(id).style.top = (position.y - toolTipCanvasMargin) + "px";
...
function findTooltip() {
	...
	var vector = new THREE.Vector3((mouse.x / window.innerWidth) * 2 - 1, -(mouse.y / window.innerHeight) * 2 + 1, 1);
	var ray;
	if (stereoDisp) {
		vector.unproject(cameraLeft);
		ray = new THREE.Raycaster(camera.position, vector.sub(cameraLeft.position).normalize());
	}
	else {
		...
		if (intersects[index].object != INTERSECTED) {
			...
			if (Object.keys(orbitalTimes).length !== 0 && shipSurfaceMesh) {
				...
				if (intersects[index].object.name != "ship") {
					...
					INTERSECTED.material.color.setHSL(colour.h, colour.s, colour.l);
	...
	else // there are no intersections
	{
		...
		if (document.getElementById("toolTip").style.display == "inline" || document.getElementById("toolTip").style.display == "none") {
...
function updateTooltipText() { // Update the text of the main tooltip
	// As long as there is a valid intersected object - that isn't the ship
	if (intersectedObject && intersectedObject.name && intersectedObject.name != "ship") {
		...
		if (expanseShow && intersectedObject.logo) {
			...
			printer = "<div style='font-size: 1.7em;'>" + intersectedObject.name + "</div><br>";
			printer += "<img src = 'assets/logos/" + intersectedObject.logo + ".png' style = 'max-width:200px;max-height:200px'><br><br>"
			printer += "<div style='font-size: 1.2em;'>" + intersectedObject.company + "</div>";
		}
		else {
			// Set the current name
			var name = intersectedObject.name
			if (planets[name] || name == "Sol") { // If it's a planet (or the Sun)

				// Set the initial colour (for the sun)
				var colour = "#ffff99"
				var smallHeadingColour = "#ffffcc"
				if (name != "Sol") { // If it's an actual planet
				...
				printer = "<div style='font-size: 1.5em; color: " + colour + ";'>" + name + "</div><br><br><div style='font-size: 1.2em;'>";
				...
				if (name != "sun") {
					...
					var distance = magnitude(subVec(mainPosition, currentPositions[center]))

					// Set the title
					printer += "<span " + styleInsert + ">Distance from " + center + "</span><br> "

					// Display the distance - and if it's to small, display it in kilometers
					if (distance < 0.001) {
						printer += round(distance * convertDistance("AU", "KM"), 4) + " KM<br>";
					}
					else {
					...
					// Find the velocity
					var velocity = magnitude(findVelocity(name, currentTime)) * convertSpeed("AU/Y", "KM/S")

					// Display the current velocity
					printer += "<span " + styleInsert + ">Current Velocity</span><br>" + round(velocity, 4) + " KM/S<br>";

					// Calculate and display the solar irradiance  of the selected planet
					printer += "<span " + styleInsert + ">Solar irradiance</span><br> " + round(1367 / Math.pow(magnitude(mainPosition), 2), 2) + " W/m<sup>2</sup><br>";
					...
					if (center != "sun") { // Put it in KM if it's a moon - it's too small otherwise
						printer += "<span " + styleInsert + ">Semi-major axis</span><br>" + round(planets[name]["a"] * convertDistance("AU", "KM"), 4) + " KM<br>";
					}
					else { // Put it in AU if it's a planet
						printer += "<span " + styleInsert + ">Semi-major axis</span><br>" + round(planets[name]["a"], 4) + " AU<br>";
					}
					...
					if (planets[name]["r"] && !planets[name]["satellite"]) {
						...
						if (planets[name].mapClass) {
							if (planets[name].mapClass.model) { // If it's a model - add a caveat that says effective radius, because it's not spherical
								insert = " [effective]"
							}
						}
						// Display the radius
						printer += "<span " + styleInsert + ">Radius" + insert + "<br></span>" + round(planets[name]["r"] * convertDistance("AU", "KM"), 0) + " KM<br>";
					}

					// If it has an effective mass and isn't a satellite, display the surface (or "surface" for gas giants) gravity
					if (planets[name]["gravParam"] && !planets[name]["satellite"]) {
						// Calculate and display surface gravity
						printer += "<span " + styleInsert + ">Surface gravity</span><br>" + round((findGravParam(name) / Math.pow(planets[name]["r"] * convertDistance("AU", "M"), 2)) / 9.81, 2) + " g<br>";
					}

				}
				else { // Display the scripted sun ones
					printer += "<span " + styleInsert + ">Mass<br></span> 1.989 x 10<sup>30</sup> kg<br>";
					printer += "<span " + styleInsert + ">Surface Temperature<br></span> 5778 K<br>";

					printer += "<span " + styleInsert + ">Radius<br></span> 695508 km<br>";
					printer += "<span " + styleInsert + ">Age<br></span> 4.5 billion years<br>";
				}

				// Close off the div
				printer += "</div>"
			}
			else { // Not in the planets array
				var name = intersectedObject.name
				...
				if (stars[name]) {
					...
					printer += "Distance: " + stars[name]["distance"] + " light years"
				}
			}
		}
...
function updateTooltip() { // Update the graphics of the tooltip to move correctly
...
function updateTooltipGraphics(position, id) { // Update the graphics of the specified tooltip

	// Set the height of the tooltip
	var toolHeight = document.getElementById(id + "Content").clientHeight
	var tooltipLineFlat = Math.pow(document.getElementById(id + "Content").clientHeight * 10, 0.5);

	if (toolHeight < 200 && intersectedObject) { // If the image hasn't loaded yet, just format it like it has already
		if ((expanseShow && intersectedObject.logo)) {
	...
	var canvasWidth = tooltipLineFlat + toolHeight * Math.tan(DtoR(tooltipLineAngle)) + toolTipCanvasMargin
	...
	else { // Mouse in right half of screen
	...
	var finalHeight;
	var bottomCanvas;
	if ((position.y > window.innerHeight / 2 - toolTipMargin) || (document.getElementById(id).style.bottom != "" && position.y > window.innerHeight / 2 + toolTipMargin)) { // In bottom half of screen (or over the sun - to keep consistent)
		finalHeight = toolHeight
		bottomCanvas = true;
	}
	else { // In top half of screen
		finalHeight = 0;
		bottomCanvas = false;
	}
	...
	var flatTooltipX = toolTipCanvasMargin + tooltipLineFlat + (toolHeight * Math.tan(DtoR(tooltipLineAngle)))
	var flatBendX = toolTipCanvasMargin + (toolHeight / 2 * Math.tan(DtoR(tooltipLineAngle)))

	// Figure out where everything needs to go
	var radius = 10;
	var circleX = flatBendX + (radius * Math.cos(DtoR(tooltipLineAngle)) + radius * Math.tan(DtoR(tooltipLineAngle)) * (1 - Math.sin(DtoR(tooltipLineAngle))))
	var circleY = flatY
	var startAngle;
	var endAngle
	if (bottomCanvas) {
	...
	ctx.arc(subtractor + valueModifier * (circleX), circleY, radius, DtoR(startAngle), DtoR(endAngle), (leftCanvas && bottomCanvas) || (!leftCanvas && !bottomCanvas));

	// Create the angled part of the line
	ctx.lineTo(subtractor + valueModifier * (0 + toolTipCanvasMargin), toolTipCanvasMargin + finalHeight);
	...
	var endRadius = 4
	ctx.arc(subtractor + valueModifier * (0 + toolTipCanvasMargin), toolTipCanvasMargin + finalHeight, endRadius, 0, 2 * Math.PI);
	ctx.fill();
}

function updateShipTooltipText() { // Update the text of the ship tooltip
	...
	var printer = "<div style='font-size: 1.1em; color: " + colour + ";'>Ship</div><br><div style='font-size: 0.9em;'>";
	...
	var velocity = 0;
	if (shipEndTime) { // If it's ballistic, use the existing function
		velocity = magnitude(findVelocity("ship", currentTime)) * convertSpeed("AU/Y", "KM/S")
	}
	else if (ISEndTime) { // If it's interstellar, just use the computation functions
		// Collect data
		var a = ISTransferData["data"]["accel"];
		var c = 299792458;
		var t = (currentTime.getTime() - ISStartTime.getTime()) * convertTime("MS", "S");

		if (t > ISTransferData["data"]["accelTime"]) { // Move to coast mode if it is past accelerating
			t = ISTransferData["data"]["accelTime"];
		}
		velocity = calculateISVelocity(a, t, c) * convertSpeed("M/S", "KM/S")
	}
	else if (EpsteinEndTime) { // If it's Epstein, find where it is a short while afterwards, and then use distance/time

		var timeMargin = 3;
		var newTime = new Date(currentTime.getTime() + timeMargin)

		var firstPos = calculateEpsteinPosition(currentTime, EpsteinTransitData)
		var secondPos = calculateEpsteinPosition(newTime, EpsteinTransitData)

		var velocity = magnitude(subVec(secondPos, firstPos)) * convertDistance("AU", "KM") / (timeMargin * convertTime("MS", "S"))
	}

	// Display the velocity
	printer += "<span " + styleInsert + ">Current Velocity</span><br>" + round(velocity, 4) + " KM/S<br>";
	if (ISEndTime) { // If it's interstellar, display the cee-fractional velocity
		printer += round(velocity * convertSpeed("KM/S", "C") * 100, 4) + " % C <br>"
	}
	...
	printer += "<span " + styleInsert + ">Remaining time" + timeInsert + "</span><br>" + round(remainingTime, 1) + " " + units + "<br>";
...
function updateShipTooltip() { // Update the position and graphics of the ship tooltip
...
function updateSim() {
	...
	if (!runningTour) {
		...
		else if (cameraMode == "ship") { // Update for ship view mode

			controlShipViewTarget();
			hideShipsAfterTrans();

		}
		else if (cameraMode == "shipCenter") { // This is basically orbital, but centered on the ship because the ship is special
		...
		if ((cameraMode == "orbital" || cameraMode == "shipCenter") && planets["ship"]) {
		...
		if (nightEarth) { // If the night side of Earth rendered, manage its display
			manageEarthNight();
		}
...
function updateDisplayTime() {
	...
	var clockTime;
	if (timeIncrement > (convertTime("D", "S"))) { // If it's moving faster than 1 day/sec, just show the date
		clockTime = day + " / " + month + " / " + year;
	}
	else {
		clockTime = hours + " : " + minutes + "  " + day + " / " + month + " / " + year;
	}
...
function updateLoadingScreens() { // This keeps the auxiallary screens mirrored to the primaries
	document.getElementById("bootScreenAux").style.display = document.getElementById("bootScreen").style.display
	document.getElementById("smallLoadingScreenAux").style.display = document.getElementById("smallLoadingScreen").style.display
	document.getElementById("captionAux").style.display = document.getElementById("caption").style.display

	document.getElementById("captionAux").innerHTML = document.getElementById("caption").innerHTML
}

function updateAllTooltips() { // Update all the tooltips
	...
	if ((cameraMode == "orbital" || cameraMode == "shipCenter") && planets["ship"]) {
		...
		if (showingShipTooltip) {
			...
			var cen = addVec(lastShipLocation, currentPositions[planets["ship"].center]);
			...
			if (angle >= Math.PI / 2 || a >= b) { // If the camera is not looking at the ship, hide the tooltip
	...
	else { // Hide the ship tooltip if there is no ship
		document.getElementById("shipToolTip").style.display = "none"
	}
}
...
function moveSecondary3DCameras() { // Move and align the 3D cameras

	// Find normal state vectors
	var camVec = reverseThreeVector(camera.position); // Sun -> Primary Camera
	var bodyCamVec = subVec(multiplyVec(totalScale, currentPositions[centeredObject]), camVec); // Chosen body -> Primary Camera

	// Find and moderate the size of the planet
	var centralSize = Math.pow(planets["earth"]["r"] / planets[centeredObject]["r"], 0.01)

	if (cameraMode == "shipCenter") { // If it's the ship, moderate it instead
		centralSize = planets["earth"]["r"] / planets["ship"]["surfaceMesh"].scale.x * Math.pow(10, -40);
	}
	...
	if (bodyCamVec[0] * bodyCamVec[1] < 0) { // In quadrant 2 or 4, flip the direction to keep it consistent, as the maths reverses in those quadrants
...
function updateSpotLight() { // Update the position and angle of the spotlight

	// Find where the spotLight should point, and set the target
	var lineVector = reverseThreeVector(planets[centeredPlanet]["markerMesh"].position);
	...
	if (cameraMode == "shipCenter" && planets["ship"]["center"] == "sun") {
		lineVector = reverseThreeVector(planets["ship"]["surfaceMesh"].position);
	...
	if (cameraMode == "shipCenter" && planets["ship"]["center"] == "sun") {
		...
		distanceValue = 2209.789810047425 * shipSurfaceMesh.scale.z * bufferValue / totalScale
	}
	...
	if (cameraMode == "shipCenter" && planets["ship"]["center"] == "sun") {
		spotLight.shadow.camera.lookAt(planets["ship"]["surfaceMesh"].position.x, planets["ship"]["surfaceMesh"].position.y, planets["ship"]["surfaceMesh"].position.z)
	}
	else if (planets[centeredPlanet]["surfaceMesh"]) {
		spotLight.shadow.camera.lookAt(planets[centeredPlanet]["surfaceMesh"].position.x, planets[centeredPlanet]["surfaceMesh"].position.y, planets[centeredPlanet]["surfaceMesh"].position.z)
	}
...
function manageCameraNear() { // Control near part of the camera to get as close as possible while preventing the sun from displaying twice due to glitch
	...
	if (planets[centeredObject] && centeredObject) {
		...
		controls.minDistance = scene.scale.x * (planets[centeredObject]["r"] * mod) * totalScale + camera.near;
	}
}
...
function moveCameraChase() {
	...
	var differenceVector = multiplyVec(totalScale, subVec(currentPositions[centeredObject], lastPosition));

	// Move the camera the same amount
	camera.position.add(threeVector(differenceVector));

	// Change controls center to selected object
	controls.target.copy(threeVector(multiplyVec(totalScale, currentPositions[centeredObject])));
	if (!(!nauvooLaunched && launchTimer)) {
		controls.update();
	}
}
...
function checkTimeRatio() { // This system outputs the ratio of actual time to simulation "time"
...
function addTime() { // Add time to system
	...
	if (planets["sun"]) {
		...
		if (!isNaN(planets["sun"]["surfaceMesh"].rotation.z)) {
			planets["sun"]["surfaceMesh"].rotation.z = (2 * Math.PI * (timeIncrement * convertTime("MS", "D")) / (24)) + planets["sun"]["surfaceMesh"].rotation.z;
		}
		else {
...
function manageTimedDelays() { // After a certain amount of time that's in sync with the time updates, update the lightlag and tooltip
...
function updateAllShips() { // Update the ship position if applicable

	if (shipEndTime != false) { // If ballistic transfer, use ballistic update function
		updateShipPath();
	}

	if (EpsteinEndTime != false) { // If Epstien transfer, use Epstien update function
		updateEpsteinPath();
	}

	if (ISEndTime != false) { // If interstellar transfer, use interstellar update function
		updateISTransfer();
	}
}
...
function manageShipCenter() {
	...
	var differenceVector = multiplyVec(totalScale, subVec(currentShipPosition, lastPosition));

	// Move the camera to keep up
	camera.position.add(threeVector(differenceVector));

	// Change controls center to object
	controls.target.copy(threeVector(multiplyVec(totalScale, currentShipPosition)));
	if (!(!nauvooLaunched && launchTimer)) {
		controls.update();
	}
...
function manageShipNear() { // Manage what happens when you get near to the ship
	...
	if (shipAxis.x != 0) { // This is done to not be influenced by the Lorentz contraction (do note that this is invalid the moment the shipAxis is [1,0,0])
	...
	if (cameraMode == "shipCenter") {
		controls.minDistance = scene.scale.x * (shipDist) * totalScale + camera.near;
	}

	// Find out what the camera distance is
	var cameraDist = magnitude(subVec(reverseThreeVector(camera.position), reverseThreeVector(planets["ship"]["markerMesh"].position)))

	// Do not allow for zooming in beneath the ship marker if in low res
	if (lowRes && cameraMode == "shipCenter") {
		controls.minDistance = scene.scale.x * planets["ship"].markerMesh.scale.x * factor * totalScale + 1;
	}
	...
	if (cameraDist < planets["ship"].markerMesh.scale.x * factor * totalScale * (3 / 8) && cameraMode == "shipCenter") {
	...
	else if (cameraDist < planets["ship"].markerMesh.scale.x * factor * totalScale * (3 / 4) && cameraMode == "shipCenter" && !lowRes) { // If in the middle, fade the orbit out

		var opacity = (cameraDist - planets["ship"].markerMesh.scale.x * factor * totalScale * (3 / 8)) / (planets["ship"].markerMesh.scale.x * factor * totalScale * (3 / 8))
		if (EpsteinEndTime != false) {
				...
				planets["ship"]["orbitMesh"].traverse(function (node) {
					if (node.material) {
						if (ISEndTime) {
							...
							node.material.opacity = node.material.opacity * opacity;
						}
					}
				});
			}
		...
		if (EpsteinEndTime != false && !webVR) {
			...
			if (planets["ship"]["orbitMesh"] && (ISEndTime != false || shipEndTime != false) && !webVR) {
				planets["ship"]["orbitMesh"].visible = !realSystem;
			}
		}

	}
	else { // Show the orbit mesh
		if (EpsteinEndTime != false && !webVR) {
			EpsteinLine.visible = !realSystem;
			flipMarker.visible = !realSystem;
...
function hideShipsAfterTrans() { // Hide all ships once it's finished

	if (EpsteinEndTime) { // Hide the Epstien data
		EpsteinLine.visible = false;
		flipMarker.visible = false;
	}

	if (ISEndTime) { // Hide the IS transfer line
		planets["ship"]["orbitMesh"].visible = false;
	}

	if (shipEndTime) { // Hide the actual orbit for ballistic transfers
		planets["ship"]["orbitMesh"].visible = false;
	}
...
function updateSystemPosition() {
	...
	var cameraCenter = multiplyVec(-1 * scene.scale.x, reverseThreeVector(controls.target))
...
function updateAllPlanets() {
	...
	for (var name in planets) { // Start at 1 because the sun is at 0
		if (name != "sun") {
			updatePlanet(name);
		}
...
function updatePlanet(name) { // Move planet to correct position and rotation
	...
	var shipBool;
	if (name != "ship" && name != "orbit1" && name != "orbit2" && name != "orbit") { // This is to stop potentially invalid planets slipping in and throw new Error("Something bad happened.")ing errors
	...
	if (planets[name]["rotation"] != null) {
		...
		if (planets[name]["tidallyLocked"]) {
			...
			var position = [currentPositions[name][0] - currentPositions[center][0], currentPositions[name][1] - currentPositions[center][1], 0];

			// Rotate the planet
			var rotation = DtoR(angleBetweenVectors(position, positionAtEpoch)) + planets[name]["rotEpoch"]
			if (axis.y != 0) {
				planets[name]["surfaceMesh"].rotation.set(planets[name]["surfaceMesh"].rotation.x, rotation, planets[name]["surfaceMesh"].rotation.z);
			}
			else {
				planets[name]["surfaceMesh"].rotation.set(planets[name]["surfaceMesh"].rotation.x, planets[name]["surfaceMesh"].rotation.y, rotation);
			}
			...
			var rotation = 2 * Math.PI * (timeIncrement * convertTime("MS", "D")) / (planets[name]["rotation"]);
...
function recenterOrbits() { // Recenter moon orbits when parent planet moves

	for (var planet in currentPositions) { // For each planet
		if (planet != "sun" && planet != "ship") { // Not the sun and the ship - they are special
			...
			planets[planet]["orbitMesh"].position.copy(threeVector(multiplyVec(totalScale, centerCoords)));
		}
	}
}

function updatePlanetRings() { // Move planet rings and atmospheres

	var atmoSpeed = 4 / 3; // Ratio of atmosphere speed to planet speed

	for (var planet in planets) { // Iterate through all the planets

		if (planets[planet]["ringMesh"]) { // If there is a ring, keep it with the planet
			planets[planet]["ringMesh"].position.copy(threeVector(multiplyVec(totalScale, currentPositions[planet])));
		}
		...
		var rotation = 2 * Math.PI * (timeIncrement * convertTime("MS", "D")) / (planets[planet]["rotation"]);

		if (planets[planet]["atmoMesh"]) { // Keep atmosphere with planet and rotate correctly

			planets[planet]["atmoMesh"].position.copy(threeVector(multiplyVec(totalScale, currentPositions[planet])));

			if (planets[planet]["axialTilt"]) { // This ensures that it rotates correctly if the planet has an axal tilt

				planets[planet]["atmoMesh"].rotateOnAxis(axis, rotation * atmoSpeed);

			}
		}

		if (planet == "mars" && planets[planet]["atmoMesh"]) { // Simulate Martian dust storms by varying opacity

			// Set the current opacity
			planets["mars"]["atmoMesh"].material.opacity = marsOpacity;

			if (marsOpacityTicks <= 0) { // Once it reaches a target - select a new one
				...
				for (var value in marsOpacityValues) {
					...
					for (var i = 0; i < Math.round(marsOpacityValues[value]); i++) {
						marsOpacityArray.push(value);
					}
				}
				marsOpacityTarget = Number(marsOpacityArray[Math.floor(Math.random() * marsOpacityArray.length)]);
				marsOpacityValues[marsOpacityTarget] = 0;
				marsOpacityTicks = Math.random() * 60 * convertTime("D", "MS", 1);
			...
			marsOpacity += (marsOpacityTarget - marsOpacity) / (marsOpacityInterval / timeIncrement);
...
function manageOrbitOpacities() { // Manage the fading of the orbit lines
	for (var name in planets) {
		if (name != "sun" && planets[name]["rL"] && planets[name].orbitMesh) { // As long as it is a real planet...
			if (planets[name].orbitMesh.visible) { // And the orbit is visible - save on calculations
				...
				if (name == "ship") {
					...
					relPos = subVec(currentPositions[name], currentPositions[planets[name]["center"]])
				}
				else {
				...
				if (planets[name]["center"] != "sun") {
					var innerName = Object.keys(planets[planets[name]["center"]]["moons"])[Math.floor((Object.keys(planets[planets[name]["center"]]["moons"]).length - 1) / 2)]
					var innerPos = subVec(currentPositions[innerName], currentPositions[planets[name]["center"]])
					baseSpeed = Math.sqrt(M3S2toAU3Y2(findGravParam(planets[innerName]["center"])) * (((2 / magnitude(innerPos)) - (1 / planets[innerName]["a"]))))
				}

				// Calculate the number of degrees to lighten - proportional to the speed/base speed
				var degrees = rotationDegrees * Math.pow(Math.sqrt(M3S2toAU3Y2(findGravParam(planets[name]["center"])) * (((2 / magnitude(relPos)) - (1 / planets[name]["a"])))) / baseSpeed, 1.25);
				...
				if (rotationMode) {
					...
					planets[name].orbitMesh.setRotationFromAxisAngle(threeVector(planets[name]["orbitalAxis"]), DtoR(currentDegree % (1 / orbitOpacityRes)))
				}

				for (var index in planets[name].orbitMesh.children) { // Iterate through each section of the orbit
					// Establish the current degree position
					var degree = (index) * (360 / planets[name].orbitMesh.children.length)
					if (rotationMode) { // This keeps it always behind for ease of rotation
						degree = degree + (1 * (360 / planets[name].orbitMesh.children.length))
					}
					else { // This keeps it around the planet for a better look
						degree = degree + (0.5 * (360 / planets[name].orbitMesh.children.length))
					}

					// Figure out what the shading value should be based on existing parameters
					var shadingValue = 0
					if ((360 + (currentDegree - degree) * negativeComp) % 360 < degrees) {
						shadingValue = (degrees - ((360 + (currentDegree - degree) * negativeComp) % 360)) / degrees
					}
					...
					var selectedOpacity = (1 - baseOrbitOpacity) * shadingValue + baseOrbitOpacity

					// Set the opacity
					planets[name].orbitMesh.children[index].material.opacity = selectedOpacity
				}
			}
		}
	}
...
function manageSunFlare() { // Hide the sunflare if the camera is on the other side. This prevents dual flares forming.
	...
	if (centeredPlanet != "sun" || cameraMode == "shipCenter") {
		...
		if (angle > Math.PI / 2 || a >= b || webVR) { // If the camera is not looking at the sun, hide the flare
			sunFlare.visible = !lowRes;
		}
		else {
			sunFlare.visible = false;
	...
	if (realSystem) {
		...
		for (var name in planets) {
			if (name != "sun" && name != centeredObject && shouldShow(name)) {
				...
				if (angle >= Math.PI / 2 || a >= b) { // If the camera is not looking at the planet, hide the flare
...
function manageEarthNight() { // Manage the night side of Earth
	...
	var axis = new THREE.Vector3(0, 1, 0).normalize();
	var rotation = 2 * Math.PI * (timeIncrement * convertTime("MS", "D")) / (planets["earth"]["rotation"]);
	nightEarth.rotateOnAxis(axis, rotation);
	...
	if (dist < camera.near * 2 + planets[centeredObject]["r"] * 7 * totalScale) {
...
function controlSOIOrbitDisplay() { // Control camera controls to hide planetary markers and orbit lines
	...
	if (webVR) {
		...
		cam = subVec(reverseThreeVector(camera.position), multiplyVec(1 / scene.scale.x, reverseThreeVector(scene.position)))
	}
	...
	if (centeredObject != "sun" && centeredObject != "ship") {
		...
		if (dist < planets[centeredObject]["SOI"] * totalScale * (1 / 2) || dist < camera.near * 2 + planets[centeredObject]["r"] * multiplier * 15 * totalScale) { // Hide the orbit line if it gets close
			planets[centeredObject]["orbitMesh"].visible = false;

			checkEasterEggs();
		}
		else if (dist < planets[centeredObject]["SOI"] * totalScale && !lowRes) { // Fade it out as it approaches
			planets[centeredObject]["orbitMesh"].visible = !realSystem;
			planets[centeredObject]["orbitMesh"].traverse(function (node) {
				if (node.material) {
					node.material.opacity = node.material.opacity * (dist - planets[centeredObject]["SOI"] * totalScale / 2) / (planets[centeredObject]["SOI"] * totalScale / 2);
				}
			});
		}
		else if (dist < camera.near * 2 + planets[centeredObject]["r"] * multiplier * 30 * totalScale && !lowRes) { // Show it if it's far away
			planets[centeredObject]["orbitMesh"].visible = !realSystem;
			planets[centeredObject]["orbitMesh"].traverse(function (node) {
				if (node.material) {
					node.material.opacity = node.material.opacity * (dist - camera.near * 2 - planets[centeredObject]["r"] * multiplier * 15 * totalScale) / (planets[centeredObject]["r"] * multiplier * 15 * totalScale);
				}
			});
		}
		else { // Show the orbit line if it gets outside the boundary
			planets[centeredObject]["orbitMesh"].visible = !realSystem;
		}
		...
		if (planetDist < planets[centeredPlanet]["SOI"] * totalScale || planetDist < camera.near * 2 + planets[centeredPlanet]["r"] * multiplier * 15 * totalScale) {
			planets[centeredPlanet]["orbitMesh"].visible = false;
		}
		else if (planetDist < planets[centeredPlanet]["SOI"] * totalScale * 2 && !lowRes) { // Fade it out as it approaches
			planets[centeredPlanet]["orbitMesh"].visible = !realSystem && !webVR;
			planets[centeredPlanet]["orbitMesh"].traverse(function (node) {
				if (node.material) {
					node.material.opacity = node.material.opacity * (planetDist - planets[centeredPlanet]["SOI"] * totalScale) / (planets[centeredPlanet]["SOI"] * totalScale);
				}
			});
		}
		else if (planetDist < camera.near * 2 + planets[centeredObject]["r"] * multiplier * 30 * totalScale && !lowRes) { // Hide the orbit line if it's too far out
			planets[centeredPlanet]["orbitMesh"].visible = !realSystem && !webVR;
			planets[centeredPlanet]["orbitMesh"].traverse(function (node) {
				if (node.material) {
					node.material.opacity = node.material.opacity * (planetDist - camera.near * 2 - planets[centeredPlanet]["r"] * multiplier * 15 * totalScale) / (planets[centeredPlanet]["r"] * multiplier * 15 * totalScale);
				}
			});
		}
		else {
...
function managePlanetSunFlares() {
	...
	if (sunFlare.isSprite) {
		totalSize = totalSize * (dist * totalScale) * DtoR(camera.fov) / HEIGHT;
	}
	...
	var scaleFactor = 30
	planets["sun"]["markerMesh"].scale.copy(threeVector([dist / scaleFactor, dist / scaleFactor, dist / scaleFactor]));

	for (var name in planets) { // Iterate through all the planets
		if (name != "sun") { // Excluding the sun because it is special and handled earlier

			if (planets[name]["lensFlare"]) { // Don't do lens flares if not needed to

				// Set the default size
				size = HEIGHT * 0.015;

				if (planets[name]["center"] != "sun") { // If it's a moon, make it look smaller
					size = size / 3;
				}
				...
				var sunDistvalue;
				if (name == "ship") {
					sunDistvalue = (planets["earth"]["a"] / magnitude(lastShipLocation));
				}
				else {
					sunDistvalue = (planets["earth"]["a"] / magnitude(currentPositions[name]));
				}
				if (planets[name]["center"] != "sun" && planets[name]["center"] != undefined) {
					sunDistvalue = (planets["earth"]["a"] / magnitude(currentPositions[planets[name]["center"]])) * 2;
				}

				// Calculate size of planet moderator
				var planetSizevalue = 0.5;
				if (name != "ship") {
					planetSizevalue = planets[name]["r"] / planets["earth"]["r"];
				}
				...
				var resultantSize = size * planetSizevalue * sunDistvalue / value;
				...
				if (planets[name].lensFlare.isSprite) {
					resultantSize = resultantSize * (dist * totalScale) * DtoR(camera.fov) / HEIGHT;
				}


				// Set the size
				planets[name].lensFlare.scale.set(pixelRatio * resultantSize, pixelRatio * resultantSize, pixelRatio * resultantSize)
...
function controlShipViewTarget() {
	...
	var shipPosition = new THREE.Vector3().copy(planets["ship"]["markerMesh"].position);
	camera.position.copy(shipPosition);

	// Look at whichever planet is closer
	if (ISEndTime == false && shipCameraTo && shipCameraFrom) {
		if (planets[shipCameraTo]["markerMesh"].position.distanceTo(planets["ship"]["markerMesh"].position) > planets[shipCameraFrom]["markerMesh"].position.distanceTo(planets["ship"]["markerMesh"].position)) {

			// Look at the planet it came from
			controls.target.copy(planets[shipCameraFrom]["markerMesh"].position);
	...
	else { // Otherwise just look at Earth - IS transfer
...
function animate() { // Request animation frame
...
function resumeAnimation() { // Kickstart the animation again
...
function haltAnimation() { // Stop the ThreeJS animation
...
function render() { // Start the renderer
	...
	if (stereoDisp) { // Split the screen in two, one for each camera if 3D
	...
	else { // Otherwise, render normally - just one scene with one camera
...
function onWindowResize() { // ThreeJS: deal with screen resize
...
function findPlanetLocation(name, time) { // Deliver a planet location given the current time
	...
	var tempEpoch = EPOCH
	if (planets[name]["epoch"]) { // If a custom epoch, use that
	...
	while (remainder < 0) { // Find the remainder from epoch
		remainder += period;
	}
	...
	var percentageAlong = (remainder - orbitalTimes[name][previousDegree]) / (orbitalTimes[name][nextDegree] - orbitalTimes[name][previousDegree]);
	if (isNaN(percentageAlong)) { // if an error is throw new Error("Something bad happened.")n, just pick halfway
		percentageAlong = 0.5;
	}
...
function findPlanetDegree(name, position) { // This entire thing is reverse-deriving it by the same method used to generate the initial coords
	...
	var testXOne = distance * (Math.cos(o) * Math.cos(degreesFromAN) - Math.sin(o) * Math.sin(degreesFromAN) * Math.cos(i));
	var testYOne = distance * (Math.sin(o) * Math.cos(degreesFromAN) + Math.cos(o) * Math.sin(degreesFromAN) * Math.cos(i));

	var degreesFromANTwo = (Math.PI - degreesFromAN) % (2 * Math.PI)

	var testXTwo = distance * (Math.cos(o) * Math.cos(degreesFromANTwo) - Math.sin(o) * Math.sin(degreesFromANTwo) * Math.cos(i));
	var testYTwo = distance * (Math.sin(o) * Math.cos(degreesFromANTwo) + Math.cos(o) * Math.sin(degreesFromANTwo) * Math.cos(i));
...
function findVelocity(name, time) { // Return velocity at a given time of a planet
	...
	var velMag = Math.sqrt(M3S2toAU3Y2(findGravParam(planets[name]["center"])) * (2 / magnitude(position) - 1 / planets[name]["a"]))
...
function generateOrbitalCoords(name) { // Iterate through and calculate all orbital positions for a planet and store them.
...
function generateOrbitalTimes(name) { // Calculate where the planet should be at a given time
	...
	while (counter < 360 * ORBIT_RESOLUTION) {
		...
		var velocity = Math.pow(gravitationalParameter * ((2 / magnitude(arrayOne)) - (1 / a)), 0.5);
...
function calculateOrbitalPositionVector(name, degree) { // Given which degree a planet is at, return the position vector
	...
	var distance = a * (1 - e * e) / (1 + e * Math.cos(DtoR(eccentricDegree)));
	...
	var x = distance * (Math.cos(o) * Math.cos(degreesFromAN) - Math.sin(o) * Math.sin(degreesFromAN) * Math.cos(i));
	var y = distance * (Math.sin(o) * Math.cos(degreesFromAN) + Math.cos(o) * Math.sin(degreesFromAN) * Math.cos(i));
	...
	if (center != "sun" && name != "luna" && name != "the moon" && name != "ship") {
		...
		if (planets[center]["axialTilt"]) {
			...
			iAxis = DtoR(iAxis);
			x = distance * (Math.cos(oAxis) * Math.cos(degreesFromAxisAN) - Math.sin(oAxis) * Math.sin(degreesFromAxisAN) * Math.cos(iAxis));
			y = distance * (Math.sin(oAxis) * Math.cos(degreesFromAxisAN) + Math.cos(oAxis) * Math.sin(degreesFromAxisAN) * Math.cos(iAxis));
...
function calculateEffectiveParams(name) {
	...
	if (axialTilt[0] == 0 && axialTilt[1] == 0) { // If it points straight up, deflect marginally to avoid divide by zero errors
	...
	if (n[1] < 0) { // Flip to loAN if it is around the way, the inverse cos function can't explain everytihing
		axisAN = 360 - axisAN;
	}
...
function calculateAxialTilt(name) { // Rotate the angular momentum vector by the difference between vertical and the center's axis to find the new angular momentum vector
...
function calculateLambertTransfer(nameOne, nameTwo, distOne, distTwo) { // Calculate the transfer between two planets
	...
	var gravitationalParameter = parseScalarInput(findGravParam(center), "m^3/s^2");

	// Calculate periods
	var periodOne = parseScalarInput(findPeriod(aOne.value * convertDistance("M", "AU"), center), "y");
	var periodTwo = parseScalarInput(findPeriod(aTwo.value * convertDistance("M", "AU"), center), "y");
	var synodicPeriod = parseScalarInput(calculateSynodicPeriod(periodOne.value * convertTime("S", "Y"), periodTwo.value * convertTime("S", "Y"), center), "y"); // Synodic Period in years
	...
	var outerPoint = multiply(largeSemiMajor, add(parseScalarInput(1, ""), largeE));
	var innerPoint = multiply(smallSemiMajor, subtract(parseScalarInput(1, ""), smallE))
	...
	var periodT = parseScalarInput(findPeriod(transitA.value * convertDistance("M", "AU"), center), "y");
	...
	if ((timeRatio + lastTimeRatio) / 2 > 1.5 && !twoStage) {
		console.log("Calculating Low Resolution Transfer - Slow Running Computer")
		resolution = 100;
	}
	totalCalculations = 0;
	...
	var timeIncrement = divide(multiply(parseScalarInput(upperTransitBound - lowerTransitBound, ""), periodT), parseScalarInput(resolution, ""));
	...
	for (var deptTime = 0; deptTime < synodicPeriod.value; deptTime += increment.value) { // Iterate through departure times - once it reaches the synodic period, it should have found one

		// Clear out a window so it can assign more transfers
		transitWindows[deptTime] = {};

		for (var travelTime = periodT.value * lowerTransitBound; travelTime < periodT.value * upperTransitBound; travelTime += timeIncrement.value) { // Iterate through transit times
			setTimeout(function (deptTime, travelTime) { // Set timeout to allow program to function for computation - parameters passed at other end



				calculateTransferWindow(deptTime, travelTime, nameOne, nameTwo, calcData)

			},
				0, deptTime, travelTime); // This passes the parameters inside so it doesn't calculate 40,000 of just the final variables
		}
	}
	...
	if (twoStage) {
		...
		setTimeout(function () { // Wait for the transfer calculator to finish up BEFORE running this - it must have found the best one
			secondTransferStage(nameOne, nameTwo, increment.value, timeIncrement.value, miscData, calcData)
		}, 0);
	}
	else {
		setTimeout(function () { // Wait for the transfer calculator to finish up BEFORE running this - it must have found the best one
...
function calculateTransferWindow(deptTime, travelTime, nameOne, nameTwo, miscData) { // Calculate an individual transfer window
	...
	var departingTime = new Date(startTime + deptTime * convertTime("S", "MS", 1));
	var rOne = parseVectorInput(findPlanetLocation(nameOne, departingTime), "AU");
	var departingVelocity = parseVectorInput(findVelocity(nameOne, departingTime), "AU/y");

	// Find initial variables for second planet
	var arrivingTime = new Date(departingTime.getTime() + outputScalar(travelTime, "ms"));
	var rTwo = parseVectorInput(findPlanetLocation(nameTwo, arrivingTime), "AU");
	var arrivingVelocity = parseVectorInput(findVelocity(nameTwo, arrivingTime), "AU/y");
	...
	// Find deltaV
	var deltaV = parseScalarInput(angleBetweenVectors(outputVector(rOne, "AU"), outputVector(rTwo, "AU")), "degrees");
	...
	if (deltaV.value > Math.PI) { // These are limits defined by the maths, just roll with it
	...
	var t1 = calculateTime(p1, rOne, rTwo, k, l, m, deltaV, gravitationalParameter)
	var t2 = calculateTime(p2, rOne, rTwo, k, l, m, deltaV, gravitationalParameter)
	...
	while (Math.abs(t2.value - travelTime.value) > tofMargin.value && !isNaN(p2.value) && numTries >= 0 && p2.value < high.value && p2.value > low.value) {
		var pnew = add(p2, divide(multiply(subtract(travelTime, t2), subtract(p2, p1)), subtract(t2, t1))); // Linear convergence

		// Update variables for the next go
		p1 = p2;
		p2 = pnew;
		t1 = t2;
		t2 = calculateTime(p2, rOne, rTwo, k, l, m, deltaV, gravitationalParameter);
		numTries -= 1;
	...
	var g = divide(multiply(multiply(rOneMag, rTwoMag), sin(deltaV)), pow(multiply(gravitationalParameter, p), 0.5));
	//var g = rOneMag * rTwoMag * Math.sin(deltaV) / Math.pow(gravitationalParameter * p, 0.5);
	var fDot = multiply(multiply(pow(divide(gravitationalParameter, p), 0.5), tan(divide(deltaV, 2))), (subtract(subtract(divide(subtract(1, cos(deltaV)), p), divide(1, rOneMag)), divide(1, rTwoMag))));
	//var fDot = Math.pow(gravitationalParameter / p, 0.5) * Math.tan(deltaV / 2) * (((1 - Math.cos(deltaV)) / p) - (1 / rOneMag) - (1 / rTwoMag));
	var gDot = subtract(1, multiply(divide(rOneMag, p), subtract(1, cos(deltaV))));
	...
	if (nameOne == "orbit1") {
		...
		DTOGrav = parseScalarInput(calculateEscapeVelocity(nameOne, distOne) + calculateExtraVelocity(DTO.value, nameOne, distOne), "m/s");
		deltaVee = add(deltaVee, parseScalarInput(calculateEscapeVelocity(nameOne, distOne) + calculateExtraVelocity(DTO.value, nameOne, distOne), "m/s"));
	}

	// If not aerobraking, deal with orbit and satellite possibilities
	if (!((planets[nameOne]["center"] == "sun" && document.getElementById("IPAero").checked) || (planets[nameOne]["center"] != "sun" && document.getElementById("ILAero").checked))) {
		if (nameOne == "orbit2") {
			...
			DCOGrav = parseScalarInput(calculateEscapeVelocity(nameTwo, distTwo) + calculateExtraVelocity(DCO.value, nameTwo, distTwo), "m/s");
			deltaVee = add(deltaVee, parseScalarInput(calculateEscapeVelocity(nameTwo, distTwo) + calculateExtraVelocity(DCO.value, nameTwo, distTwo), "m/s"));
		}
	}

	// Calculate the semi-major axis for verification
	//var a = (m * k * p) / (((2 * m) - Math.pow(l, 2)) * Math.pow(p, 2) + (2 * k * l * p) - Math.pow(k, 2));
	var a = divide(multiply(multiply(m, k), p), (add(multiply(subtract(multiply(2, m), pow(l, 2)), pow(p, 2)), subtract(multiply(multiply(k, 2), multiply(l, p)), pow(k, 2)))))
	...
	formatData["timeDiff"] = Math.abs(t2.value - travelTime.value) / travelTime.value * 100 + "%";
	...
	if (!transitWindows[deptTime]) { // Open up the departure time JSON if not initialised
		transitWindows[deptTime] = {};
	}
	...
	if (validTransfer(formatData)) {
		...
		if (twoStage && !inSecondStage) {
			...
			if (storedWindows.length < windowsNum) {
				...
				storedWindows.sort(function (x, y) { // This sorts is in descending order, highest first
					x = x.deltaVee
					y = y.deltaVee
					if (x < y) {
						return 1;
			...
			if (deltaVee.value < lowestDeltaVee.value) { // If it's better, set it as the new best
	...
	if ((new Date().getTime() / 1000) % 0.5 < 0.1) {
		if (timeEstimateUpdated == false) {
			var elapsedTime = (new Date().getTime() - transTime.getTime()) / 1000
			var predictedTime = elapsedTime * (synodicPeriod.value - deptTime) / deptTime
			lastTimeMessage = "<br><br>Estimated time remaining<br>" + round(predictedTime) + " s"
	...
	if (seconds < 1) { // Update the message after 4 seconds
		if (messageUpdated == false) {
			message = loadingMessages[Math.floor(Math.random() * loadingMessages.length)];
			messageUpdated = true;
		}
	...
	// Update loading text
	var displayText = "Calculating Transfer<br>" + round(100 * deptTime / synodicPeriod.value, 0) + "%"
	...
	// Otherwise, keep adding dots
	document.getElementById("wittyText").innerHTML = message + ".".repeat(Math.floor(seconds));
	document.getElementById("wittyTextAux").innerHTML = message + ".".repeat(Math.floor(seconds));
}

function calculateTime(p, rOne, rTwo, k, l, m, deltaV, gravParam) { // Given the parameters, calculate the transfer time for the convergence algorithm
	...
	// Calculate more variables that don't have a physical correlate - see previous maths comment
	var g = divide(multiply(multiply(rOneMag, rTwoMag), sin(deltaV)), pow(multiply(gravParam, p), 0.5));
	//var g = rOneMag * rTwoMag * Math.sin(deltaV) / Math.pow(gravitationalParameter * p, 0.5);
	var fDot = multiply(multiply(pow(divide(gravParam, p), 0.5), tan(divide(deltaV, 2))), (subtract(subtract(divide(subtract(1, cos(deltaV)), p), divide(parseScalarInput(1, ""), rOneMag)), divide(parseScalarInput(1, ""), rTwoMag))));
	var deltaE = invCos(subtract(1, multiply(divide(rOneMag, a), subtract(parseScalarInput(1, ""), f))));
	if (invSin(divide(multiply(multiply(-1, rOneMag), multiply(rTwoMag, fDot)), pow(multiply(gravParam, a), 0.5))).value < 0) { // Check to see if it should flip it - in the dead zone of arccos
		deltaE.value = 2 * Math.PI - deltaE.value;
	}

	// Calculate transfer time
	var t = add(g, multiply(pow(divide(pow(a, 3), gravParam), 0.5), subtract(deltaE, sin(deltaE))));

	if (a.value < 0) { // Calculate it differently if it's hyperbolic
		var deltaF = Math.acosh(outputScalar(subtract(1, multiply(divide(rOneMag, a), subtract(1, f))), ""));
		if (deltaF < 0) {
			deltaF = 2 * Math.PI + deltaF;
		}
		t = add(g, multiply(pow(divide(multiply(-1, pow(a, 3)), gravParam), 0.5), (Math.sinh(deltaF) - deltaF)));
	}
...
function secondTransferStage(nameOne, nameTwo, dTimeIncrement, tTimeIncrement, miscData, calcData) { // Complete the second stage of the transfer calculations
	...
	for (var index in storedWindows) {
		...
		for (var dIndex = 0; dIndex < secondResolution; dIndex++) {
			...
			for (var tIndex = 0; tIndex < secondResolution; tIndex++) {
				...
				if (tIndex == 0 && dIndex == 0) { // If it's the first one, it's already been calculated

					var formatData = primaryWindow

					var deltaVee = parseScalarInput(primaryWindow.deltaVee, "M/S")
					...
					calculateTransferWindow(newDTime, newTTime, nameOne, nameTwo, calcData)
				}
			}
		}
	}
	...
	console.log("Stage 2 Complete: " + (totalCalculations - stageOneCalcs) + " windows analysed")
...
function finaliseTransferCalc() {
	...
	console.log("Lambert Transfer Calculated: " + round((new Date().getTime() - transTime.getTime()) / 1000, 2) + "s")
...
function finishLambertCalculation() { // Finalise the lambert calculation for display
	...
	if (magnitude(r) < magnitude(r2)) { // Look at the higher energy planet, tends to be more accurate (not by much, but a bit)
		selectedR = r;
		selectedV = v;
	...
	lowestData["params"] = paramsFromVec(selectedR, selectedV, selectedTime, center);
...
function paramsFromVec(r, v, time, center) { // Calculate orbital parameters given orbital state vectors and a given time
	...
	if (h[0] == 0 && h[1] == 0) { // If angular velocity is straight up, deflect negligibly to avoid divide by 0 errors
	...
	var ePartOne = multiplyVec(Math.pow(magnitude(v), 2) - (gravitationalParameter / magnitude(r)), r);
	...
	var a = 1 / ((2 / magnitude(r)) - (Math.pow(magnitude(v), 2) / gravitationalParameter));
	...
	var L = (360 + RtoD(Math.acos(dotProduct(e, r) / (magnitude(e) * magnitude(r))))) % 360;
	...
	var aoPE = RtoD(Math.acos(dotProduct(n, e) / (magnitude(n) * magnitude(e))));
	if (e[2] < 0) {
		aoPE = 360 - aoPE
	}
...
function calculateEscapeVelocity(centerBody, initalRadiusAbove) { // Calculate the escape velocity of a body
	...
	var deltaVee = Math.pow((2 * gravParam) / radiusTotal, 0.5) - Math.pow(gravParam / radiusTotal, 0.5);

	return deltaVee;
}

function calculateExtraVelocity(velocity, name, radius) { // Calculate the excess velocity at the bottom of a hyperbolic trajectory given the velocity at insertion
	...
	var exitSOIA = 1 / ((2 / planetSOI) + (Math.pow(insertionVelocity, 2) / SOIGravParam));
	...
	var deltaVeeExit = Math.pow(SOIGravParam * ((2 / radiusTotal) + (1 / exitSOIA)), 0.5);
...
function calculateIPTransfer() { // Calculate an interplanetary transfer
	...
	var properNameTwo = document.getElementById("toTarget").value.replace('The ', '');
	...
	if (nameOne == nameTwo) { // Stop them fron transferring between the same planets - doesn't really work
...
function delayIPTransfer() { // Set timeouts to get the results afterwards - Lambert uses these to stop program shutdown during computation
	if (twoStage) {
		setTimeout(function () {
			setTimeout(function () {
				finishIPTransfer()
...
function finishIPTransfer() { // Finish off the IP Transfer
	...
	maneuvers.push({ // Add NAME Transfer orbit in the maneuvers list
	...
	var DV = calculateEscapeVelocity(nameTwo, toRadius) + deltaVeeExit + deltaVeeEnter + calculateEscapeVelocity(nameOne, fromRadius);
...
function calculateILTransfer() { // Calculate an interlunar transfer
	...
	if (shipEndTime) { // Stop transfers in progress, if any
		endShipTransit();
	}
	...
	var properNameOne = document.getElementById("fromILTarget").value.replace('The ', '');
	var properNameTwo = document.getElementById("toILTarget").value.replace('The ', '');
	...
	if (nameOne == nameTwo && nameOne != "orbit") { // Make sure they don't pick the same moon
		...
		if (nameOne == "orbit") {
			aOne = planets[center]["r"] + (document.getElementById("fromDistanceAboveIL").value * convertDistance("KM", "AU", 1));
		}
		else {
			aOne = planets[nameOne]["a"];
		}
		if (nameTwo == "orbit") {
			aTwo = planets[center]["r"] + (document.getElementById("toDistanceAboveIL").value * convertDistance("KM", "AU", 1));
		}
		else {
...
function delayILTransfer() { // Set timeouts to get the results afterwards - Lambert uses these to stop program shutdown during computation
	if (twoStage) {
		setTimeout(function () {
			setTimeout(function () {
				finishILTransfer()
...
function finishILTransfer() { // Finish off the IL Transfer
	...
	if (nameTwo == "orbit2" || planets[nameTwo]["satellite"] == true) {
		if (nameOne != "orbit1" && !planets[nameOne]["satellite"]) {
			...
			var deltaVeeExit = calculateExtraVelocity(returnData["TO"], nameOne, fromRadius);
			maneuvers.push({
		...
		if (nameOne == "orbit1" || planets[nameOne]["satellite"] == true) {
			...
			deltaVeeExit = calculateExtraVelocity(returnData["TO"], nameOne, fromRadius);
			maneuvers.push({
				name: properNameTwo[0] + "TO",
				title: properNameTwo + " Transfer Orbit",
				deltaVee: (deltaVeeExit)
	...
	if (nameTwo == "orbit2") {
		...
		deltaVeeExit = calculateExtraVelocity(returnData["CO"], nameTwo, fromRadius);
...
function findShipLocation() { // Find where the ship should be at a given time
...
function printPorkchop(bestDeptTime, bestTransitTime, minDV) { // Print the entire porkchop plot
	...
	if (!loggedTransfer && !twoStage) {
		...
		if (Object.keys(transitWindows).length == 100) {
			for (var deptTimeIndex in originalDeptKeys) { // Iterate through all departure times
				...
				var nextDeptTime = Number(originalDeptKeys[Number(deptTimeIndex) + 1])
				var newDeptTime = Number(nextDeptTime + deptTime) / 2

				transitWindows[newDeptTime] = {}

				for (var transitTimeIndex in originalTransKeys) { // Iterate through all 200x200 transits
					var leftColumn = transitTime == 0;
					var rightColumn = transitTime == originalTransKeys.length - 1

					var transitTime = Number(originalTransKeys[Number(transitTimeIndex)]);


					var nextTransitTime = Number(originalTransKeys[Number(transitTimeIndex) + 1]);
					...
					if (!rightColumn && !bottomRow) {
						...
						if (!bottomRow) {
							deltaVees.push(transitWindows[nextDeptTime][transitTime])
						}
						...
						for (var index in deltaVees) {
							if (deltaVees[index]) {
								...
								if (!validTransfer(deltaVees[index]) && !deltaVees[index].filler) {
									totalDV = Infinity
			...
			for (var deptTimeIndex in originalDeptKeys) { // Iterate through all departure times
				...
				for (var transitTimeIndex in originalTransKeys) { // Iterate through all 200x200 transits
					...
					if (!rightColumn && !bottomRow) {
						...
						if (!rightColumn) {
							deltaVees.push(transitWindows[deptTime][nextTransitTime])
						}
		...
		var width = document.getElementById("porkchop").width / Object.keys(transitWindows).length;
		var height = document.getElementById("porkchop").height / Object.keys(transitWindows["0"]).length;
		...
		for (var deptTimeIndex in deptKeys) { // Iterate through all departure times
			var deptTime = deptKeys[deptTimeIndex]

			for (var transitTimeIndex in transitKeys) { // Iterate through all 200x200 transits
				...
				if (!isNaN(deptTime) && !isNaN(transitTime)) {
					...
					var curDV = transitWindows[deptTime][transitTime]["deltaVee"];

					// Select correct colour number
					var index = 0;
					var value = curDV / minDV;
					while (value >= Number(Object.keys(colours)[index])) { // Select colour range
						index += 1;
					}
					...
					var isValid = validTransfer(transitWindows[deptTime][transitTime]) || transitWindows[deptTime][transitTime].filler

					// Calculate colour
					if (isValid && index == 0) { // If it's the smallest, use the lowest colour

						currentColour = colours[Object.keys(colours)[0]];
					}
					else if (isValid && index < Object.keys(colours).length) { // Find the colour calculated

						for (var i = 0; i < 3; i++) {
							currentColour[i] = calculateColour(colours[minBound][i], colours[maxBound][i], minBound, maxBound, curDV / minDV);
						}

					}
					else { // If it's invalid, pick the largest colour, as it failed (and is likely a hyperbola - v. high delta vee)

						currentColour = colours[Object.keys(colours)[Object.keys(colours).length - 1]]
					}

					// Place a pixel on the porkchop
					ctx.fillStyle = "rgb(" + currentColour[0] + "," + currentColour[1] + "," + currentColour[2] + ")";
					ctx.fillRect(Math.ceil(x), Math.ceil(y), Math.ceil(width), Math.ceil(height));
					y -= height;
				}
			}
		...
		var centerY = document.getElementById("porkchop").height - (transitKeys.indexOf(String(bestTransitTime)) * height);
		...
		if (centerX > document.getElementById("porkchop").width / 2) { // Flip depending on which side the best is
		...
		for (var i in colours) { // Print the key
			var DVrange = round(minDV * Number(i), 0);
			if (i == Object.keys(colours)[Object.keys(colours).length - 1]) {
				DVrange += "+";
			}

			var colour = "rgb(" + colours[i][0] + "," + colours[i][1] + "," + colours[i][2] + ")";
			printer += '<tr><td><svg width="12px" height="12px"><g><rect height="18px" width="18px" style="fill:' + colour + ';"></rect></g></svg></td><td>' + DVrange + '</td></tr>';
		}

		document.getElementById("porkchopKey").innerHTML = printer;

		for (var deptTime in transitWindows) { // Iterate through all departure times

			for (var transitTime in transitWindows[deptTime]) { // Iterate through all 200x200 transits
...
function calculateColour(start, end, min, max, cur) { // Calculate the correct shade: 
...
function logCourse(x, y) { // This is a debugging function - logs transfer data of clicked area on porkchop plot to console
	// Given X and Y of mouse click event (relative to screen)
	var boundingRect = document.getElementById("porkchop").getBoundingClientRect();
	...
	if (displayLoggedCourse) {
		if (validTransfer(storedData)) {
			...
			swal("Sorry", "You can't take a look at this transfer. Please try another!", "warning")
		}
	}
	else {
...
function validTransfer(transferData) {
	return !isNaN(transferData.misc.p) && transferData.numTries > 0 && transferData.a > 0 && transferData.misc.p < transferData.misc.highP && transferData.misc.p > transferData.misc.lowP
}

// Ballistic UI Display Functions

function displayManeuvers(maneuvers, transitDetails) { // Display the Delta V's calculated
	...
	printer += "<tr><td><h3>Maneuver</h3></td><td><h3>&Delta;V (m/s)</h3></td></tr>";
	for (var index in maneuvers) { // Run through all of the maneuvers
		if (maneuvers[index]["deltaVee"] > 0.001) { // Hide any negligible maneuvers (1mm/s is negligible)

			// No tooltip unless the maneuver has a title - in which case it's the title
			var toolTip = "";
			if (maneuvers[index]["title"] != undefined) { // Add a tooltip if specified
				toolTip = maneuvers[index]["title"];
			}

			var colour = "#ffffff";
			if (maneuvers[index]["hide"] == true) { // Colour maneuvers paler if it is hidden/irrelevant (like for aerobraking)
				colour = "#888888";
			}
			else {
			...
			printer += "<tr title='" + toolTip + "' style='color:" + colour + "'><td title='" + toolTip + "'>" + maneuvers[index]["name"] + "</td><td title='" + toolTip + "'>" + round(maneuvers[index]["deltaVee"], 3) + "</td></tr>";
	...
	printer += "<tr><td>Total</td><td>" + totalDeltaVee + "</td></tr></table></td></tr>";

	printer += "<tr><td><table>"
	printer += "<tr><td><h3>Transit Time</h3></td></tr>"

	if (planets[transitDetails.nameOne].center == "sun") {
		printer += "<tr><td>" + round(transitDetails["tTime"] * convertTime("Y", "Mo"), 1) + "</td><td>months</td></td></tr>"
	}
	printer += "<tr><td>" + round(transitDetails["tTime"] * convertTime("Y", "W"), 1) + "</td><td>weeks</td></td></tr>"
	printer += "<tr><td>" + round(transitDetails["tTime"] * convertTime("Y", "D"), 1) + "</td><td>days</td></td></tr>"
	if (planets[transitDetails.nameOne].center != "sun") {
		printer += "<tr><td>" + round(transitDetails["tTime"] * convertTime("Y", "H"), 1) + "</td><td>hours</td></td></tr>"
	}
	printer += "</table></td></tr>"
...
function calculateShipCoords(shipParams) { // Calculate the ship's coordinates
...
function changeILCenter() { // Change central planet for an IL transfer - changes center as well

	// Set the center
	var centeredObject = document.getElementById("ILCenter").value.toLowerCase();
		...
		if (shouldShow(name)) {
			if (name != "sun" && centeredObject != "sun" && name != "ship") {
				if (planets[name]["center"] == centeredObject) {
					printer += "<option value='" + capitalise(name) + "'>" + capitalise(name) + "</option>";
				}
			}
		}
	}
...
function startShipDisplay(startTime, endTime) { // Intitialize the ship display
...
function showShipPath(startTime, endTime) { // Show the ship path and the ship's geometry
	...
	var markerMesh = new THREE.Mesh(geometry, new THREE.MeshBasicMaterial({
		color: shipColour,
	...
	if (shipParameters["center"] != "sun") { // Adjust scaling if not centered on the sun
		var mod;
		if (shipAxis.x != 0) {
...
function updateShipPath() { // Update where the ship's path, location and rotation
	...
	planets["ship"]["markerMesh"].position.copy(threeVector(multiplyVec(totalScale, addVec(shipArray, currentPositions[shipCenter]))));
	if (planets["ship"]["lensFlare"]) {
		planets["ship"]["lensFlare"].position.copy(planets["ship"]["markerMesh"].position);
	}
	planets["ship"]["surfaceMesh"].position.copy(planets["ship"]["markerMesh"].position);

	// Set the rotation angle based on how far through it is
	shipRotAngle = 2 * Math.PI * (currentTime.getTime() - shipStartTime.getTime()) / (shipEndTime.getTime() - shipStartTime.getTime())
	...
	planets["ship"]["markerMesh"].quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), threeVector(directionVector).clone().normalize());
	planets["ship"]["surfaceMesh"].quaternion.setFromUnitVectors(shipAxis, threeVector(directionVector).clone().normalize());
	if (planets["ship"].surfaceMesh.children[0]) {
		if (planets["ship"].surfaceMesh.children[0].children[0].children[0]) { // If it's the Roci, turn it to make it look better
			planets["ship"].surfaceMesh.children[0].children[0].children[0].quaternion.setFromAxisAngle(new THREE.Vector3(0, 0, 1), shipRotAngle)
		}
	}

	//  Move the orbit path to keep with the central body
	planets["ship"]["orbitMesh"].position.copy(threeVector(multiplyVec(totalScale, currentPositions[shipCenter])));

	// If it's outside the transfer time, stop the transit
	if (currentTime.getTime() > shipEndTime.getTime() || currentTime.getTime() < shipStartTime.getTime()) {
		endShipTransit();

	}

	if (planets["ship"]) { // If the ship still exists (it has gotten halfway through only to have the ship deleted from under it)

		if (timeIncrement != 0) { // Switch to different tracking system if no time's passing
			earlierShipLocation = lastShipLocation;
			lastShipLocation = findShipLocation();
		}
		else { // Just keep it going normally
...
function endShipTransit() { // Stop all ship transits and reset
	...
	if (cameraMode == "shipCenter") {
		...
		if (planets["ship"].center != "sun") {
			document.getElementById("centerSelect").value = planets["ship"].center;
			changeCenter();
			if (shouldShow(transitData["nameTwo"])) {
				document.getElementById("moonSelect").value = transitData["nameTwo"];
				changeMoon();
			}
		}
		else {
			...
			if (shouldShow(transitData["nameTwo"])) {
				document.getElementById("centerSelect").value = transitData["nameTwo"];
			}
			else {
...
function createShipOrbit() { // Create the orbit of the ship
...
function drawShipOrbit(positions) { // Return a ThreeJS geometry given the ship's orbital positions
...
function validateAccel() { // Error checking on the interstellar acceleration

	// Find the acceleration
	var accel = Number(document.getElementById("ISAccel").value);

	if (accel <= 0) { // Set to 1g - Zero doesn't work
		accel = 9.8;
	}
...
function validateDist() { // Error checking on the interstellar distance

	// Find the distance
	var dist = Number(document.getElementById("ISDist").value);

	if (dist <= 0) { // It kinda breaks with a distance of zero, set it to arbitrary non-zero
		dist = 1;
	}
...
function updateTargetDistance() { // Update the target distance if a star selected
	...
	if (!document.getElementById("ISCoast").checked) { // If it's not coasting, limit the time as needed
		timeLimits();
	}
	...
	if (document.getElementById("starTarget").options[document.getElementById("starTarget").selectedIndex].text == "Homestead II") {
		document.getElementById("DVTitle").innerHTML = "Why the Starship <i>Avalon</i> Makes No Sense";
		document.getElementById("maneuverDisplay").innerHTML = "<a href='https://docs.google.com/document/d/1yrxS7CCBUR1JSzk-vZSBew5YpE2Bp-3RSavM3KmGRPM/edit?usp=sharing' target='_blank '>Link to Google Doc</a>";
		document.getElementById("deltaVeeDisplay").style.display = "block";
	}
}

function timeLimits() { // Limit the time if coasting, set the time if not

	// Gather data
	var flyby = document.getElementById("ISFlyby").checked;
	var coast = document.getElementById("ISCoast").checked;
	var accelTime = Number(document.getElementById("ISTime").value) * convertTime("Y", "S", 1);;
	var accel = Number(document.getElementById("ISAccel").value);
	var dist = Number(document.getElementById("ISDist").value) * convertDistance("LY", "M", 1);
	var c = 299792458;

	if (!flyby) { // If it's not doing a flyby, max is defined by accel-decel, rather than accel
		dist = dist / 2;
	}
	...
	else if (accelTime > maxTime) { // Set it to the max allowable time if it's over
		newAccelTime = maxTime;

	}
	else { // Just set it to what it was already
		newAccelTime = accelTime;
	}

	// Set the time
	document.getElementById("ISTime").value = newAccelTime * convertTime("S", "Y", 1);

	if (coast == false) { // If it's not coasting, fix that more properly to be exactly as needed
		updateCoastTime();
	}
}

function updateCoastTime() { // Calculate coasting time
	...
	if (!flyby) { // If it's not doing a flyby, it will accel-decel, rather than just a straight accel
		dist = dist / 2;
	}
	...
	if (document.getElementById("ISTime").disabled) { // Correctly update the right colour if it's disabled or not
	...
	document.getElementById("ISTime").value = round(accelTime * convertTime("S", "Y", 1), 11);
}

// Interstellar UI Display

function printISTransfer(transferData) { // Print the IS Transfer data - each part is pretty self-explanatory
	var printer = "<tr><td><table style='text-align:left;'>";

	// Print key
	printer += "<tr><td>" + "<b>Phase</b>" + "</td><td>" + "<b>Distance (ly)</b>" + "</td><td>" + "<b>Earth Time (y)</b>" + "</td><td>" + "<b>Ship Time (y)</b>" + "</td></tr>";

	// Print table headings
	printer += "<tr><td>" + "Accel" + "</td><td>" + round(transferData["distances"]["accel"], 3) + "</td><td>" + round(transferData["times"]["accel"], 3) + "</td><td>" + round(transferData["timesRel"]["accel"], 3) + "</td></tr>";

	// Print accel, coast (if applicable), and decel
	if (transferData["times"]["coast"] > Math.pow(10, -10)) {
		printer += "<tr><td>" + "Coast" + "</td><td>" + round(transferData["distances"]["coast"], 3) + "</td><td>" + round(transferData["times"]["coast"], 3) + "</td><td>" + round(transferData["timesRel"]["coast"], 3) + "</td></tr>";
	}
	if (transferData["times"]["decel"] > 0) {
		printer += "<tr><td>" + "Decel" + "</td><td>" + round(transferData["distances"]["decel"], 3) + "</td><td>" + round(transferData["times"]["decel"], 3) + "</td><td>" + round(transferData["timesRel"]["decel"], 3) + "</td></tr>";

	}

	// Log total
	printer += "<tr><td>" + "Total" + "</td><td>" + round(transferData["distances"]["total"], 3) + "</td><td>" + round(transferData["times"]["total"], 3) + "</td><td>" + round(transferData["timesRel"]["total"], 3) + "</td></tr>";

	// End table
	printer += "</table></td></tr>";

	// Print maximum velocity
	printer += "<tr><td>Maximum Velocity: " + round(transferData["maxVel"] * 100, 2) + "% c</td></tr>";

	var value = (transferData["times"]["accel"] + transferData["times"]["decel"]) * convertTime("Y", "S") * transferData["data"]["accel"]
	var power = 0;
	while (value / Math.pow(10, power + 1) > 1) {
		power += 1
	}

	printer += "<tr><td>Effective &Delta;V: " + round(value / Math.pow(10, power), 3) + "x10<sup>" + power + "</sup> m/s</td></tr>";

	// Set general maneuvers
	document.getElementById("DVTitle").innerHTML = "Time and Distance Breakdown";
	document.getElementById("maneuverDisplay").innerHTML = printer;
	document.getElementById("deltaVeeDisplay").style.display = "block";
}
...
function calculateISTransfer() { // Primary IS Transfer Calculator
	...
	if (flyby != true) { // If it's not doing a flyby, it will need to decelarate
		decelTime = accelTime;
		totalTime += decelTime;
	...
	var coastTimeRel = coastTime / Math.pow(1 - Math.pow(velocity / c, 2), -0.5);
	totalTime += coastTime;
	...
	if (document.getElementById("ISDist").value == document.getElementById("starTarget").value) { // Use the real star data if available
		starName = document.getElementById("starTarget").options[document.getElementById("starTarget").selectedIndex].text;
		RA = stars[starName]["RA"];
		DE = stars[starName]["DE"];
	}

	var transferData = { // Store a bunch of transfer data
		data: {
			accel: accel,
			accelTime: accelTime
		},
...
function displayISTransfer(transferData) { // Start displaying
	...
	var RA = (180 * transferData["star"]["RA"][0] / 12) + (180 * transferData["star"]["RA"][1] / (12 * 60)) + (180 * transferData["star"]["RA"][2] / (12 * 60 * 60));
	var DE = (180 * transferData["star"]["DE"][0] / 180) + (180 * transferData["star"]["DE"][1] / (180 * 60)) + (180 * transferData["star"]["DE"][2] / (180 * 60 * 60));
...
function startISTransfer(transferData, r) { // Start all calculations for the IS transfer
	...
	var timeToDist = Math.pow(Math.pow(d / c, 2) + 2 * d / transferData["data"]["accel"], 0.5);
	ISEndTime = new Date(currentTime.getTime() + timeToDist * convertTime("S", "MS"));
	...
	console.log("Interstellar Transfer Calculated: " + round((new Date().getTime() - transTime.getTime()) / 1000, 2) + "s");
	swal({
		title: "Transfer Calculated",
...
function updateISTransfer() { // Update the ship position during the transfer
	...
	if (planets["ship"]) {
		...
		if (ISEndTime.getTime() < currentTime.getTime()) { // Stop if it's over
			endISTransfer();
			swal({
		...
		else if (currentTime.getTime() < ISStartTime.getTime()) { // Stop if it's before the start
			endISTransfer();
		}
		...
		var currentPos = addVec(ISTransferData["path"]["start"], setMagnitude(subVec(ISTransferData["path"]["end"], ISTransferData["path"]["start"]), dist));
		...
		if (planets["ship"]) {
			...
			planets["ship"]["markerMesh"].position.copy(threeVector(multiplyVec(totalScale, shipArray)));
			if (planets["ship"]["lensFlare"]) {
				planets["ship"]["lensFlare"].position.copy(planets["ship"]["markerMesh"].position);
			}
			planets["ship"]["surfaceMesh"].position.copy(planets["ship"]["markerMesh"].position);
			...
			var directionVector = subVec(ISTransferData["path"]["end"], ISTransferData["path"]["start"]);
			...
			var size = (1 + magnitude(subVec(currentPos, currentPositions["earth"]))) * (3 * markerScale);
			size = size * planets["ship"].markerMesh.size
			planets["ship"]["markerMesh"].scale.copy(threeVector([size, size / gamma, size]));

			// Contract the ship correctly based along the primary axis
			if (reverseThreeVector(shipAxis)[0] != 0) {
				var scale = reverseThreeVector(planets["ship"]["surfaceMesh"].scale)[1];
				planets["ship"]["surfaceMesh"].scale.copy(threeVector([scale / gamma, scale, scale]));
			}
			else if (reverseThreeVector(shipAxis)[1] != 0) {
				var scale = reverseThreeVector(planets["ship"]["surfaceMesh"].scale)[0];
				planets["ship"]["surfaceMesh"].scale.copy(threeVector([scale, scale / gamma, scale]));
			}
			else if (reverseThreeVector(shipAxis)[2] != 0) {
				var scale = reverseThreeVector(planets["ship"]["surfaceMesh"].scale)[0];
				planets["ship"]["surfaceMesh"].scale.copy(threeVector([scale, scale, scale / gamma]));
			}

			// Set the rotation angle based on how far through it is
			shipRotAngle = 2 * Math.PI * (currentTime.getTime() - ISStartTime.getTime()) / (ISEndTime.getTime() - ISStartTime.getTime())
			...
			if (planets["ship"].surfaceMesh.children[0]) {
				if (planets["ship"].surfaceMesh.children[0].children[0].children[0]) { // This is so the Roci can roll to look better
					planets["ship"].surfaceMesh.children[0].children[0].children[0].quaternion.setFromAxisAngle(new THREE.Vector3(0, 0, 1), shipRotAngle)
				}
			}
		}
...
function endISTransfer() { // Stop the IS transfer
	...
	if (cameraMode == "shipCenter") { // Move it back to the sun once it's done if centered on the ship
		document.getElementById("centerSelect").value = "sun";
		changeCenter();
		updateDropdowns();
	}
...
function calculateISDistance(a, t, c) {
	...
	return Math.pow(c, 2) / a * (Math.pow(Math.pow(a * t / c, 2) + 1, 0.5) - 1)
}
...
function collectEpsteinData() { // Get all the data for an Epstien transfer
	...
	var nameOne = document.getElementById("fromEpsteinTarget").value.toLowerCase();
	var nameTwo = document.getElementById("toEpsteinTarget").value.toLowerCase();
...
function displayEpsteinManeuvers(epstienTransitData) {
	...
	maneuvers.push({
		name: "Decelerating from " + epstienTransitData["nameOne"] + " velocity",
	...
	var totalDeltaVee = 0;
	var printer = "<tr><td><table style='text-align:left;'>";
	printer = "<tr><td><h3>Maneuver</h3></td><td><h3>Time (hours)</h3></td></tr>";
	for (var index in maneuvers) {
		if (maneuvers[index]["time"] != 0) {

			maneuvers[index]["time"] = round(maneuvers[index]["time"] * convertTime("S", "H", 1), 3);

			printer += "<tr><td>" + maneuvers[index]["name"] + "</td><td>" + maneuvers[index]["time"] + "</td></tr>";
	...
	var value = totalDeltaVee * convertTime("H", "S", 1) * epstienTransitData["accel"]
	var power = 0;
	while (value / Math.pow(10, power + 1) > 1) {
		power += 1
	}

	printer += "<tr><td>Total &Delta;V: " + round(value / Math.pow(10, power), 3) + "x10<sup>" + power + "</sup></td><td> m/s</td></tr>";
	...
	console.log("Epstein Transfer Calculated: " + round((new Date().getTime() - transTime.getTime()) / 1000, 2) + "s")
}

function changeEpsteinCenter() { // Change central planet for an Epstein transfer - changes center as well

	// Change the center of the program to match the Epstien center
	var centeredObject = document.getElementById("EpsteinCenter").value.toLowerCase();
	document.getElementById("centerSelect").value = centeredObject;
	changeCenter();
	...
	for (var name in currentPositions) {
		if (shouldShow(name)) {
			if (name != "sun" && name != "ship") {
				if (planets[name]["center"] == centeredObject) {
					printer += "<option value='" + name + "'>" + capitalise(name) + "</option>";
				}
			}
		}
	}
...
function calculateEpsteinTransfer(nameOne, nameTwo, gees) { // Main Epstein calculator
	...
	if (nameOne != nameTwo) {
		...
		transferData = calculateTransferDuration((planets[nameOne]["a"] + planets[nameTwo]["a"]) * convertDistance("AU", "M", 1), accel, 0, 0);
		...
		for (var transitTime = 0; transitTime < period; transitTime += period / 10000) { // Iterate through to find valid transfer
			...
			var arrivingTime = new Date(currentTime.getTime() + transitTime * convertTime("Y", "MS", 1));
			...
			var distance = magnitude(subVec(arrivingPosition, departingPosition)) * convertDistance("AU", "M", 1);
			...
			var alignAngle = Math.acos(dotProduct(alignVector, departingVelocity) / (magnitude(alignVector) * magnitude(departingVelocity)));

			// Find the planet velocity aligned with the transit vector
			var alignedDeparting = magnitude(departingVelocity) * Math.cos(alignAngle);
			var antiDeparting = magnitude(departingVelocity) * Math.sin(alignAngle);

			alignVector = subVec(departingPosition, arrivingPosition);
			alignAngle = Math.acos(dotProduct(alignVector, arrivingVelocity) / (magnitude(alignVector) * magnitude(arrivingVelocity)));

			var alignedArriving = magnitude(arrivingVelocity) * Math.cos(alignAngle);
			var antiArriving = magnitude(arrivingVelocity) * Math.cos(alignAngle);
			...
			if (alignedDeparting < 0) {
				antiDeparting = Math.sqrt(Math.pow(alignedDeparting, 2) + Math.pow(antiDeparting, 2));
				alignedDeparting = 0;
			}

			if (alignedArriving < 0) {
				antiArriving = Math.sqrt(Math.pow(alignedArriving, 2) + Math.pow(antiArriving, 2));
				alignedArriving = 0;
			}

			// Find out how long the transfer would take
			var transferData = calculateTransferDuration(distance, accel, alignedDeparting, alignedArriving);
			...
			var difference = Math.abs(arrivingDuration + departingDuration + travelTime - transitTime * convertTime("Y", "S", 1));

			// Find the transfer that most closely aligns with the selected iteration
			if (difference < bestTimeDifference) { // Store all data about best transfer
				bestTimeDifference = difference;
				bestTransitData["accel"] = accel;
				bestTransitData["transitTime"] = transitTime;
				bestTransitData["calculatedTransitTime"] = arrivingDuration + departingDuration + travelTime;
		...
		if (planets[nameOne]["center"] == "sun") { // Do not get too close to the sun
			maxDist = 0.275;
		}
		...
		if (dirVec1 == dirVec2) { // If the closest distance is outside of the transfer line, it's in no danger
			maxDist = 0;
		}
		...
		if (magnitude(subVec(centralProject, centralObj)) > maxDist) { // If it's okay...
			// Start displaying best transfer
			showEpsteinPath(bestTransitData);
			displayEpsteinManeuvers(bestTransitData);
			endSmallLoading();
		}
		else { // If it gets too close, add some time and try again
			var timeToAdd = findPeriod(smallestA, planets[nameOne]["center"]) * convertTime("Y", "MS") / 10;
			currentTime.setTime(currentTime.getTime() + timeToAdd);
			collectEpsteinData();
		}

	}
	else { // Tell the user they must go BETWEEN places
		swal("Sorry", "You must pick two different bodies", "error");
	}
}

function calculateTransferDuration(distance, accel, startVelocity, endVelocity) { // Calculate how long a transfer should take, given data
	...
	if (startVelocity > endVelocity) { // Figure out which is larger
	...
	if (startVelocity > endVelocity) { // Add the time to start with
	...
	distance = distance - ((accel / 2) * Math.pow(timeRemoved, 2) + (timeRemoved * lowestStartVelocity));
	...
	preFlipTime += (-largestStartVelocity + Math.sqrt(Math.pow(largestStartVelocity, 2) + accel * distance)) / accel;
	postFlipTime += (-largestStartVelocity + Math.sqrt(Math.pow(largestStartVelocity, 2) + accel * distance)) / accel;
...
function showEpsteinPath(epstienTransitData) { // Display the path of the Epstien transfer
	...
	var transferStart = new Date(currentTime.getTime() + epstienTransitData["times"][1] * convertTime("S", "MS", 1));
	var transferEnd = new Date(arrivingTime.getTime() - epstienTransitData["times"][4] * convertTime("S", "MS", 1));
	...
	var flipDistFromStart = epstienTransitData["startVelocity"] * epstienTransitData["returnData"]["preFlip"] + (epstienTransitData["accel"] / 2) * Math.pow(epstienTransitData["returnData"]["preFlip"], 2);
	...
	geometry.vertices.push(
		threeVector(multiplyVec(totalScale, findPlanetLocation(nameOne, startTime))),
		threeVector(multiplyVec(totalScale, findPlanetLocation(nameOne, transferStart)))
	);

	geometry.vertices.push(
		threeVector(multiplyVec(totalScale, findPlanetLocation(nameOne, transferStart))),
		threeVector(multiplyVec(totalScale, findPlanetLocation(nameTwo, transferEnd)))
	);

	geometry.vertices.push(
		threeVector(multiplyVec(totalScale, findPlanetLocation(nameTwo, transferEnd))),
		threeVector(multiplyVec(totalScale, findPlanetLocation(nameTwo, endTime)))
	);
...
function startEpsteinDisplay(epstienTransitData) { // Begin main display for Epstien, initialize ship

	// Figure out roughly how large to make the ship
	var a = (planets[epstienTransitData["nameOne"]]["a"] + planets[epstienTransitData["nameTwo"]]["a"]) / 2;
	...
	EpsteinEndTime = new Date(currentTime.getTime() + epstienTransitData["transitTime"] * convertTime("Y", "MS", 1));
...
function updateEpsteinPath() { // Update Epstien path position

	// Get data
	var transitData = EpsteinTransitData;

	// If outside transfer, end it
	if (currentTime.getTime() < EpsteinStartTime.getTime() || EpsteinEndTime.getTime() < currentTime.getTime()) { // Stop if outside transfer time
		endEpsteinTransfer();
	}

	// Get times needed for update
	var transferStart = new Date(EpsteinStartTime.getTime() + transitData["times"][1] * convertTime("S", "MS", 1));
	var transferEnd = new Date(transitData["arrivingTime"].getTime() - transitData["times"][4] * convertTime("S", "MS", 1));
	...
	var startLocation = findPlanetLocation(nameOne, transferStart);
	var endLocation = findPlanetLocation(nameTwo, transferEnd);
	var flipDistFromStart = transitData["startVelocity"] * transitData["returnData"]["preFlip"] + (transitData["accel"] / 2) * Math.pow(transitData["returnData"]["preFlip"], 2);
	flipDistFromStart = flipDistFromStart * convertDistance("M", "AU", 1);
	var travelVector = subVec(endLocation, startLocation);
	var flipLocation = addVec(startLocation, multiplyVec(flipDistFromStart / magnitude(travelVector), travelVector));

	if (planets["ship"]) { // Deal with ship visiblity
		...
		EpsteinArray = calculateEpsteinPosition(currentTime, EpsteinTransitData);

		if (currentTime.getTime() < transferStart.getTime()) { // If matching velocity, follow the planet
			EpsteinArray = findPlanetLocation(nameOne, currentTime);
			EpsteinArray = addVec(EpsteinArray, setMagnitude([-EpsteinArray[1], EpsteinArray[0], EpsteinArray[2]], 10 * planets[nameOne]["r"]))
		}
		else if (transferEnd.getTime() < currentTime.getTime()) {
			EpsteinArray = findPlanetLocation(nameTwo, currentTime);
			EpsteinArray = addVec(EpsteinArray, setMagnitude([-EpsteinArray[1], EpsteinArray[0], EpsteinArray[2]], 10 * planets[nameTwo]["r"]))
		}

		// Move the ship marker
		lastShipLocation = EpsteinArray
		currentShipPosition = addVec(EpsteinArray, currentPositions[EpsteinCenter]);
		planets["ship"]["markerMesh"].position.copy(threeVector(multiplyVec(totalScale, currentShipPosition)));
		planets["ship"]["lensFlare"].position.copy(planets["ship"]["markerMesh"].position);
		planets["ship"]["surfaceMesh"].position.copy(planets["ship"]["markerMesh"].position);

		// Keep the flip marker stationary relative to the central body
		flipMarker.position.copy(threeVector(multiplyVec(totalScale, addVec(flipLocation, currentPositions[EpsteinCenter]))));
		...
		if (currentTime.getTime() < transferStart.getTime()) { // Except when it is attenuating the misaligned velocity
			directionVector = multiplyVec(-1, findVelocity(nameOne, currentTime));
		}
		else if (transferEnd.getTime() < currentTime.getTime()) {
		...
		// Set the rotation angle based on how far through it is
		shipRotAngle = 2 * Math.PI * (currentTime.getTime() - transferStart.getTime()) / (transferEnd.getTime() - transferStart.getTime())

		// Rotate the ship to face the correct travel direction
		planets["ship"]["markerMesh"].quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), threeVector(directionVector).clone().normalize());
		planets["ship"]["surfaceMesh"].quaternion.setFromUnitVectors(shipAxis, threeVector(directionVector).clone().normalize());
		if (planets["ship"].surfaceMesh.children[0]) {
			if (planets["ship"].surfaceMesh.children[0].children[0].children[0]) { // If it's the Roci, turn it so it looks better
				planets["ship"].surfaceMesh.children[0].children[0].children[0].quaternion.setFromAxisAngle(new THREE.Vector3(0, 0, 1), shipRotAngle)
			}
		}

		// Keep the transfer line in the correct position
		EpsteinLine.position.copy(threeVector(multiplyVec(totalScale, currentPositions[EpsteinCenter])));
	}
}

function calculateEpsteinPosition(time, transitData) { // Given transit data and current time, where should the ship be?
	//var geometry = new THREE.Geometry();

	// Figure out when all this is happening
	var transferStart = new Date(EpsteinStartTime.getTime() + transitData["times"][1] * convertTime("S", "MS", 1));
	var flipTime = new Date(transferStart.getTime() + transitData["returnData"]["preFlip"] * convertTime("S", "MS", 1));
	var transferEnd = new Date(transitData["arrivingTime"].getTime() - transitData["times"][4] * convertTime("S", "MS", 1));
	...
	var travelVector = subVec(endLocation, startLocation);
	var flipLocation = addVec(startLocation, multiplyVec(flipDistFromStart / magnitude(travelVector), travelVector));
	var midVelocity = transitData["startVelocity"] + transitData["returnData"]["preFlip"] * transitData["accel"];

	if (time.getTime() > flipTime.getTime()) { // Currently POST-FLIP

		// Calculate distance and position
		var secondsAfter = (time.getTime() - flipTime.getTime()) * convertTime("MS", "S", 1);
		var distance = secondsAfter * midVelocity - (transitData["accel"] / 2) * Math.pow(secondsAfter, 2);
		distance = distance * convertDistance("M", "AU");
		var displacementVector = subVec(endLocation, flipLocation);
		position = addVec(flipLocation, multiplyVec(distance / magnitude(displacementVector), displacementVector));

	}
	else { // Currently PRE-FLIP

		// Calculate distance and position
		var secondsBefore = (flipTime.getTime() - time.getTime()) * convertTime("MS", "S", 1);
		distance = secondsBefore * midVelocity - (transitData["accel"] / 2) * Math.pow(secondsBefore, 2);
		distance = distance * convertDistance("M", "AU");
...
function endEpsteinTransfer() { // Stop an Epstein Transfer
	...
	if (cameraMode == "shipCenter") { // Reset the ship view to the central body if it was on the ship

		if (planets["ship"].center != "sun") {
			document.getElementById("centerSelect").value = planets["ship"].center;
			changeCenter();
			if (shouldShow(EpsteinTransitData["nameTwo"])) {
				document.getElementById("moonSelect").value = EpsteinTransitData["nameTwo"];
				changeMoon();
			}
		}
		else {
			if (shouldShow(EpsteinTransitData["nameTwo"])) {
				document.getElementById("centerSelect").value = EpsteinTransitData["nameTwo"];
			}
			else {
				document.getElementById("centerSelect").value = "sun";
			}
...
function fixLightLagDropdown() { // Correct the light lag dropdown if it was looking at the ship when a transfer ends
...
function updateLightMoon() { // Update the dropdown for the moon section of the light dropdown
	...
	if (planet != "sun" && planet != "ship") { // The sun and the ship are special
		// Iterate through all planets
		for (var moon in planets) {

			if (planets[moon]["center"] == planet) { // If it is a moon of that planet
				if (shouldShow(moon)) { // And it should be shown

					// Add it to the printer, and keep say it has been added
					printer += "<option value='" + moon + "'>" + capitalise(moon) + "</option>";
					moonAdded = true;
				}
			}
		}
...
function calculateLightLag() { // Calculate delay due to light speed for all shown bodies, and print

	// Initialise table headers
	var printer = "Propagation Delays<table style='margin:auto;text-align:left;'><tr><th><b>Planet</b></th><th><b>Delay</b></th></tr>";
	...
	if (document.getElementById("lightMoonChoice").value != "planet" && document.getElementById("lightMoonChoice").value) {
	...
	if (middle == "ship") {
		...
		if (planets[middle]["center"] != "sun") {
			middlePos = addVec(lastShipLocation, currentPositions[planets[middle]["center"]]);
		}
	}
	else {
		middlePos = currentPositions[middle];
		if (!shouldUpdate(middle)) {
			middlePos = addVec(findPlanetLocation(middle, currentTime), currentPositions[planets[middle]["center"]]);
		}
	...
	var time = round(convertTime("S", "M", 1) * convertDistance("AU", "M", 1) * (distance / lightspeed), 1);

	// Add the row to the table
	printer += "<tr><td>" + name + "</td><td>" + time + " <span style='color:#888;'>minutes</span></td></tr>";

	// Go through all the planets
	for (var name in planets) {
		if (shouldShow(name)) { // Do it for every planet
			if (planets[name]["center"] == "sun" && name != "sun") {
				...
				time = round(convertTime("S", "M", 1) * convertDistance("AU", "M", 1) * (distance / lightspeed), 1);
	...
	for (var name in planets) { // Do it for every moon of the planet (from the surface)
		if (shouldShow(name)) {
			if ((planets[name]["center"] == middle && middle != "sun") || (planets[middle]["center"] == name && name != "sun") || (planets[middle]["center"] == planets[name]["center"] && planets[name]["center"] != "sun")) {
				...
				if (!shouldUpdate(name)) {
					...
					if (planets[middle]["center"] != "sun") {
						distanceVector = subVec(addVec(findPlanetLocation(name, currentTime), currentPositions[planets[middle]["center"]]), middlePos);
					}
				}

				// Calculate distance and time
				distance = magnitude(distanceVector);
				time = round(convertDistance("AU", "M", 1) * (distance / lightspeed), 1);

				// Add the next row to the table
				printer += "<tr><td>" + name + "&nbsp;</td><td>" + time + " <span style='color:#888;'>seconds</span></td></tr>";
			}
...
function createLightLagLines() { // Create all lines for the planets (and the ship) 
...
function createLightLagLine(name) { // Create the light line
...
function displayLightLagLines(center) { // Display the lines the light travels across
	...
	if (center == "ship") {
		...
		if (planets[center]["center"] != "sun") {
			middlePos = addVec(lastShipLocation, currentPositions[planets[center]["center"]]);
		}
	}
	else {
		middlePos = currentPositions[center];
		if (!shouldUpdate(center)) {
			middlePos = addVec(findPlanetLocation(center, currentTime), currentPositions[planets[center]["center"]]);
		}
	...
	if (!realSystem) {
		for (var name in planets) {
			...
			if (centeredPlanet != "sun") {
				condition = condition || planets[centeredPlanet]["orbitMesh"].visible == true || (planets[center]["center"] == name && name != "sun") || planets[name]["center"] == center
			...
			if (condition) { // If it needs to be updated - if not, just hide it

				if (shouldShow(name) && (planets[name]["center"] == center || planets[name]["center"] == planets[center]["center"] || planets[name]["center"] == "sun")) {
					...
					if (name == "ship") {
						...
						if (planets[name]["center"] != "sun") {
							destVec = addVec(lastShipLocation, currentPositions[planets[name]["center"]]);
						}
					}
					else {
						destVec = currentPositions[name];
						if (!shouldUpdate(name)) {
							destVec = addVec(findPlanetLocation(name, currentTime), currentPositions[planets[name]["center"]]);
						}
					...
					var isMoonLine = (planets[name]["center"] != "sun" && planets[name]["center"] == planets[center]["center"]) || (planets[center]["center"] == name && planets[center]["center"] != "sun") || (planets[name]["center"] == center && planets[name]["center"] != "sun");
...
function hideLightLagLines() { // Hide all prop. delay lines
	for (var name in lightLagLines) {
		lightLagLines[name].visible = false;
	}
}

function hideLightParticles() { // Clear out the photons
...
function releaseLightParticles() { // Yes, they are photons - but this keeps a naming theme
...
function createLightParticle(name) { // Create a single photon
...
function updateLightParticles(center) { // Update the position of each photon
	var timeChange = (currentTime.getTime() - lightParticleTime.getTime()) / 1000; // Number of SIM seconds since photon release
	var lightSpeed = 299792458; // Thankfully, the exact value
	...
	for (var name in lightParticles) {
		...
		if (lightLagLines[name] && lightLagLines[name].visible) {
			...
			if (timeRate != 0) {
				var totalSimTime = lightLagLines[name].distance * convertDistance("AU", "M") / lightSpeed;
				var totalFrames = totalSimTime / (timeRate * timeScale / 1000)
			...
			var isMoonPhoton = (planets[name]["center"] != "sun" && planets[name]["center"] == planets[center]["center"]) || (planets[center]["center"] == name && planets[center]["center"] != "sun") || (planets[name]["center"] == center && planets[name]["center"] != "sun");

			if (!(planets[name]["center"] != "sun" && planets[name]["center"] == planets[center]["center"]) && !isMoonPhoton && planets[name]["center"] != "sun") {
				shouldDisplay = false;
			}
			...
			if (lightParticles[name].visible || shouldDisplay) {
				...
				// Set the size
				var size = totalScale * lightLagLines[name].distance / Math.pow(10, 20);
				if (isMoonPhoton) {
					if (planets[name]["center"] == "earth" || planets[center]["center"] == "earth") {
						size = 0.00002
					}
					else if (planets[name]["center"] == "mars" || planets[center]["center"] == "mars") {
						size = 0.000005
					}
					else {
						size = 0.00007
				...
				// Set position
				lightParticles[name].position.copy(threeVector(multiplyVec(totalScale, position)))
...
function resetShipSystems() { // Reset all types of transfers
...
function shipGeometry(scale) { // Return the ship's geometry given the scale 
	...
	// Create geometries
	var cylinderGeo = new THREE.CylinderGeometry(size / 12, size / 12, size / 4, res);
	var coneGeo = new THREE.ConeGeometry(size / 12, size / 4, res);
	var rocketGeo = new THREE.ConeGeometry(size / 15, size / 4, res);

	// Create the meshes
	var cylinderMesh = new THREE.Mesh(cylinderGeo, new THREE.MeshBasicMaterial({}));
...
function updateGlobalWebVRScaling() {
	webVRStandardLength = Math.pow(10, Number(document.getElementById("webVRScaler").value))
	updateSystemScaling()
}

// Center Changing Functions

function changeCenter() { // Change which major body is being looked at
	...
	if (document.getElementById("centerSelect").value == "ship") { // Ship display interrupt - The ship is special and rendered differently

		if (centeredPlanet != "sun" && shouldShow(centeredPlanet) && realSystem == false) {
			planets[centeredPlanet]["orbitMesh"].visible = !realSystem;
		}

		if (centeredObject != "sun" && shouldShow(centeredObject) && realSystem == false) {
			planets[centeredObject]["orbitMesh"].visible = !realSystem;
		}
		...
		var printer = "<option value='planet'>" + capitalise(centeredPlanet) + "</option>";
		...
		if (printer != "<option value='planet'>" + capitalise(centeredObject) + "</option>") {
			document.getElementById("moonSelectDisplay").style.display = "inline";
		}
		else {
			document.getElementById("moonSelectDisplay").style.display = "none";
		}
		...
		if (centeredObject != "sun" && centeredObject != "the ring" && centeredObject != "tycho station" && centeredObject && centeredObject != "ship") { // Load model if required
			if (planets[centeredObject]["surfaceMesh"].geometry.type == "SphereGeometry" && planets[centeredObject]["mapClass"]["model"]) {
				if (!lowRes) {
...
function changeMoon() {
	...
	if (document.getElementById("moonSelect").value == "ship") { // Ship centering interrupt
		...
		if (centeredObject != "sun" && centeredObject) { // Load model if not loaded
			if (planets[centeredObject]["surfaceMesh"].geometry.type == "SphereGeometry" && planets[centeredObject]["mapClass"]["model"]) {
...
function updateCameraNear() {
	// Update the camera projection matrix
	if (cameraMode == "shipCenter") {
		camera.near = 2209.789810047425 * shipSurfaceMesh.scale.y * cameraNearMultiplier / 2
	}
	else if (centeredObject == "sun" || centeredObject != centeredPlanet) {
		camera.near = 2 * totalScale / Math.pow(10, 8)
	}
	else {
		camera.near = planets[centeredObject].r * totalScale * cameraNearMultiplier
	}
...
function hideUselessMoons(oldPlanet, newPlanet) { // Strategically hide moons and surfaces to reduce computer load

	if (oldPlanet != "sun") { // The sun is special
		if (planets[oldPlanet]["surfaceMesh"]) { // If it has a surface, hide the surface and lens flare of the old planet
	...
	if (newPlanet != "sun") { // The sun is special
		...
		if ((shipCameraTo == newPlanet || shipCameraFrom == newPlanet) && cameraMode == "ship") {
	...
	if (centeredPlanet == "sun" && cameraMode == "orbital") {
		for (var name in planets) {
			if (planets[name].orbitMesh) {
				var needToShow = planets[name]["center"] == "sun" && shouldShow(name) && !realSystem
		...
		for (var name in planets) {
			if (planets[name].orbitMesh) {
				if ((planets[name]["center"] != centeredPlanet || planets[name]["center"] == "sun" || name == "ship") && (name != shipCameraFrom && name != shipCameraTo)) {
					planets[name].orbitMesh.visible = false
				}
			}
		}
		...
		for (var name in planets) {
			if (planets[name].orbitMesh) {
				if (planets[name]["center"] != centeredPlanet || planets[name]["center"] == "sun") {
					planets[name].orbitMesh.visible = false
				}
			}
		}
...
function updateSystemScaling() {
	...
	if (centeredPlanet == "sun" && cameraMode == "orbital") {
		...
		if (planets[chosenName].surfaceMesh.position && magnitude(reverseThreeVector(planets[chosenName].surfaceMesh.position)) != 0) {
			distanceModerator = magnitude(reverseThreeVector(planets[chosenName].surfaceMesh.position))
		}
		else {
		...
		if (planets[innerName].surfaceMesh.position && magnitude(reverseThreeVector(planets[innerName].surfaceMesh.position != 0))) {
	...
	var scaleValue = Number((chosenStandardLength * webVRStandardLength / distanceModerator).toPrecision(5))
...
function startSmallLoading() { // Start Small Loading Screen
	...
	if (Object.keys(orbitalTimes).length !== 0) {
		...
		smallLoadingTimer = setTimeout(function () {
			...
			document.getElementById("smallLoadingScreen").style.display = "block";
			document.getElementById("smallDisabledCover").style.display = "block";
		}, 1000); // 3000
	}
}

function endSmallLoading() { // Dismiss small loading screen

	if (smallLoadingTimer) { // If the timer actually exists

		clearTimeout(smallLoadingTimer);

		smallLoadingTimer = undefined;

		endSmallLoadingTimer = setInterval(function () { // Until it's been up for at least 3 seconds, don't dismiss it
			if (new Date().getTime() > smallLoadingCounter.getTime() + 3000) {
				document.getElementById("smallLoadingScreen").style.display = "none";
				document.getElementById("smallDisabledCover").style.display = "none";
...
function startTransferCalc() { // Stop all calculation and rendering while calculating a transfer
...
function endTransferCalc() { // Resume all animation and calculation
...
function loadSurface(name) { // Load the planetary surface from a sphere upon actually looking at it - saves on laoding everything from the start
	...
	if (name != "sun") {
		if (!lowRes) {
			if (name != "the ring") {
				if (planets[name]["rotation"] && !planets[name].surfaceMesh.material.map) {

					var loadName = name
					if (name == "the moon") { // The moon is the same as Luna
						loadName = "luna";
					}

					if (!planets[name]["mapClass"]["model"] && planets[name]["mapClass"]["surfaceMap"] != false) { // If texture, map onto surface
						planets[name].surfaceMesh.material.color.setRGB(1, 1, 1) // Colour back to white to not tint the surface
						planets[name].surfaceMesh.material.map = textureLoader.load('assets/maps/' + loadName + '.jpg');
						planets[name].surfaceMesh.material.needsUpdate = true;

					}

					if (planets[name]["mapClass"]["bumpMap"] || planets[name]["mapClass"]["normalMap"]) { // If they have height maps

						if (planets[name]["mapClass"]["normalMap"]) { // Use the normal map
							planets[name].surfaceMesh.material.normalMap = textureLoader.load('assets/normalmaps/' + loadName + '.jpg');
						}
						else { // Otherwise, bump map
							planets[name].surfaceMesh.material.bumpMap = textureLoader.load('assets/bumpmaps/' + loadName + '.jpg');
						}

						if (planets[name]["mapClass"]["normalMap"]) { // Reset the scale
							if (planets[name]["heightMapScale"]) {
								planets[name].surfaceMesh.material.normalScale = new THREE.Vector2(globalBumpScaling * planets[name]["heightMapScale"], globalBumpScaling * planets[name]["heightMapScale"]);
							}
						}
						else {
							if (planets[name]["heightMapScale"]) {
								planets[name].surfaceMesh.material.heightMapScale = globalBumpScaling * planets[name]["heightMapScale"];
								planets[name].surfaceMesh.material.bumpScale = globalBumpScaling * planets[name]["heightMapScale"];
							}
							else {
								planets[name].surfaceMesh.material.heightMapScale = globalBumpScaling * 1;
								planets[name].surfaceMesh.material.bumpScale = globalBumpScaling * 1;
							}
						}


					}
					if (planets[name]["mapClass"]["specularMap"]) { // Deal with specular specularMap - this is just Earth
						planets[name].surfaceMesh.material.specularMap = textureLoader.load('assets/specmaps/' + loadName + '.jpg');
						planets[name].surfaceMesh.material.specular = new THREE.Color(0x111111);
						planets[name].surfaceMesh.material.shininess = 30;
					}

					if (planets[name]["mapClass"]["atmo"] && name != "mars" && !lowRes) {
						planets[name].atmoMesh.material.map = textureLoader.load('assets/special/' + name + 'cloudmap.jpg')
						if (name == "earth") {
							planets[name].atmoMesh.material.alphaMap = textureLoader.load('assets/special/' + name + 'cloudmap.jpg')
						}
				...
				var texture = new THREE.TextureLoader().load('assets/special/theringedge.png');
				texture.wrapS = THREE.RepeatWrapping;
				texture.wrapT = THREE.RepeatWrapping;
				texture.repeat.set(30, 1);

				// Set the basic colour
				planets[name].surfaceMesh.children[0].material.color = { // Colour back to white to not tint the surface
					r: 1,
					g: 1,
					b: 1
				};
				planets[name].surfaceMesh.children[1].material.color = { // Colour back to white to not tint the surface
					r: 1,
					g: 1,
					b: 1
				};
				...
				texture = new THREE.TextureLoader().load('assets/special/theringinvedge.png');
				texture.wrapS = THREE.RepeatWrapping;
				texture.wrapT = THREE.RepeatWrapping;
				texture.repeat.set(30, 1);

				// Set all textures
				planets[name].surfaceMesh.children[0].material.specularMap = texture;
				planets[name].surfaceMesh.children[0].material.alphaMap = texture;
				planets[name].surfaceMesh.children[0].material.transparent = true;
				planets[name].surfaceMesh.children[1].material.map = textureLoader.load('assets/special/theringmap.png');
			}
		}
	}
}
...
function loadModel(name) {
	...
	if (planets[name]["mapClass"]["glTF"]) {
		...
		// Run the loader on the model
		loader.load('models/glTF/' + name.toUpperCase() + add + '/scene.gltf', function (loadedScene) {
			...
			setTimeout(function () {
				...
				// Iterate through all children of the container and set shadow rules
				mesh.traverse(function (child) { // Go through all the children and apply shadow rules
					if (child instanceof THREE.Mesh) {
				...
				var value = mod * planets[centeredObject].r * totalScale / (getObjectSize(planets[name]["surfaceMesh"]) / 2)
				planets[name]["surfaceMesh"].scale.copy(threeVector([value, value, value]));
		...
		loader.load('models/obj/' + name + add + '.obj', function (object) {
			...
			for (var index in object.children) { // Iterate through and merge sub-geometries into one geometry
				var child = object.children[index];
			...
			planets[name]["surfaceMesh"].scale.copy(threeVector([1, 1, 1]));
			var value = mod * planets[name].r * totalScale / (getObjectSize(planets[name]["surfaceMesh"]) / 2)
			planets[name]["surfaceMesh"].scale.copy(threeVector([value, value, value]));
		});
	}
}
...
function changeTimeScale() { // Change the rate at which time passes
	if (!isNaN(Number(document.getElementById("inputTimeScale").value))) { // If it's valid
		// Take it in and set the time frame
		apparentTimeRate = document.getElementById("inputTimeScale").value * convertTime("H", "S", 1);
		timeRate = apparentTimeRate;
		timeIncrement = timeRate * (1000 / timeScale);
	}
	else { // ELSE place the current one back in
		document.getElementById("inputTimeScale").value = apparentTimeRate * convertTime("S", "H", 1);
	}
}

function setNowTime() { // Set the time to now
	var now = new Date();

	// Get current time
	var time = new Date(now.getFullYear(), now.getMonth(), now.getDate(), now.getHours(), now.getMinutes(), now.getSeconds());
...
function setCurrentTime() { // Set the simulation time to the selected time, fix all rotations and positions
	...
	var newTime = new Date(year, month, day, hours, minutes, currentTime.getSeconds(), currentTime.getMilliseconds());
	var timeDif = newTime.getTime() - currentTime.getTime();


	for (var name in planets) {
		if (name != "ship") { // Fix rotations by however much the time just changed
			var axis = new THREE.Vector3(0, 1, 0).normalize();
			if (planets[name]["mapClass"]["model"]) {
				axis = new THREE.Vector3(0, 0, 1).normalize();
			}
			var rotation = 2 * Math.PI * timeDif * convertTime("MS", "D", 1) / (planets[name]["rotation"]);

			planets[name]["surfaceMesh"].rotateOnAxis(axis, rotation);
		}
	}
	currentTime = new Date(newTime.getUTCFullYear(), newTime.getUTCMonth(), newTime.getUTCDate(), newTime.getUTCHours(), newTime.getUTCMinutes(), newTime.getUTCSeconds());;
	displayTime = newTime;
...
function changeTransferType() { // Change type of transfer
	...
	document.getElementById(document.getElementById("transferType").value).style.display = "block";

	if (document.getElementById("transferType").value == "ILTransfer") { // Move it to the selected center if it's an IL transfer
		changeILCenter();
	}

	if (document.getElementById("transferType").value == "EpsteinTransfer") { // Move it to the selected center if it's an EP transfer
		changeEpsteinCenter();
	}
...
function hideShowPorkchop() { // Hide or show the porkchop plot
	if (porkchopShown) {
		porkchopShown = false;
...
function toIPText() { // On changing of the destination IP field - perform validation and display updates
	var name = document.getElementById("toTarget").value.toLowerCase();
	if (planets[name]["atmoMesh"] != undefined || planets[name]["r"] > planets["earth"]["r"] * 2) {
	...
	document.getElementById("toDistanceAbove").disabled = document.getElementById("IPAero").checked;

	// Grey out the textbox caption to indicate if it's disabled
	if (document.getElementById("IPAero").checked) {
		document.getElementById("toDistAboveText").style.color = "rgb(136,136,136)";
	}
	else {
		document.getElementById("toDistAboveText").style.color = "rgb(255,255,255)";
	}
}

function fromILText() { // On changing of the origin IL Field - perform validation and display updates

	if (document.getElementById("fromILTarget").value.toLowerCase() == "orbit") {
		document.getElementById("fromDistSelect").style.display = "inline";
	}
	else {
		if (planets[document.getElementById("fromILTarget").value.toLowerCase()]["satellite"]) {
...
function toILText() { // On changing of the destination IL Field - run error checking and display updates

	var name = document.getElementById("toILTarget").value.toLowerCase();

	if (name != "orbit") {
		if (planets[name]["satellite"]) { // No need for final orbit distance for satellite
		...
		if (planets[name]["atmoMesh"] != undefined) { // Aerobraking not possible without air
	...
	document.getElementById("IPAero").checked = false;
	document.getElementById("toDistanceAboveIL").disabled = document.getElementById("IPAero").checked;

	// Grey out the textbox caption to indicate that it's disabled
	if (document.getElementById("IPAero").checked) {
		document.getElementById("toDistAboveILText").style.color = "rgb(136,136,136)";
	}
	else {
		document.getElementById("toDistAboveILText").style.color = "rgb(255,255,255)";
	}
}
...
function updateAerobrakes() {
	...
	document.getElementById("toDistanceAboveIL").disabled = document.getElementById("ILAero").checked;
...
function showHideReal() {
	...
	for (var planet in planets) {
		...
		if (planet != "sun") { // Manage if lens flare and orbit should be shown (conditional on whether it can be hidden)
			planets[planet]["lensFlare"].visible = realSystem && ((planets[planet]["center"] == centeredPlanet || planets[planet]["center"] == "sun") || planet == centeredObject);

			if (planets[planet]["orbitMesh"]) {
				planets[planet]["orbitMesh"].visible = !realSystem;
			}

			if (planets[planet]["markerMesh"] && (planets[planet]["surfaceMesh"].visible || planets[planet]["center"] == "sun")) {
				if (realSystem) {
	...
	if (planets["ship"]) {
		...
		if (cameraMode == "ship") { // Hide the ship if in ship camera view
			planets["ship"].markerMesh.visible = false;
			planets["ship"].surfaceMesh.visible = false;
			planets["ship"]["lensFlare"].visible = false;
			if (planets["ship"]["orbitMesh"]) {
		...
		if (shipEndTime == false && EpsteinEndTime == false && ISEndTime == false) { // Hide ship if no transfer
			planets["ship"]["lensFlare"].visible = false;
	...
	if (realSystem) { // Switch controls
		document.getElementById("visibleControl").innerHTML = "Show Orbit Diagram";
	}
	else {

		document.getElementById("visibleControl").innerHTML = "Show Real Solar System";
	}
...
function updatePlanetsDisplay() { // Update planet display
	...
	for (var name in planets) {
		if (name != "sun" && name != "ship") {
			...
			if (planets[name].viewingClass.expanseHide || planets[name].viewingClass.minorSatellite || planets[name].viewingClass.expanse || planets[name].viewingClass.minorBody) {
...
function shouldUpdate(name) { // Determine whether the planet should be updated

	var update = false;
	if (planets[name]["surfaceMesh"].visible || planets[name]["center"] == "sun" || planets[name]["satellite"]) {
		update = true
	}
	if (planets[name]["lensFlare"]) {
...
function changePlanetDisplay(name, toggle) { // Switch if the planet should be shown or not
	...
	planets[name].surfaceMesh.visible = toggle && ((planets[name]["center"] == centeredPlanet && centeredObject != "sun") || name == centeredObject);
	if (planets[name].atmoMesh) {
		planets[name].atmoMesh.visible = toggle && (planets[name]["center"] == centeredPlanet || name == centeredObject);
	}
	planets[name].markerMesh.visible = toggle && (planets[name]["center"] == centeredPlanet || planets[name]["center"] == "sun" || name == centeredObject);

	if (planets[name].lensFlare) {
		if (realSystem == true && toggle == true) { // Show the lens flare if it should be shown AND  if realSystem = true
			planets[name].lensFlare.visible = true && ((planets[name]["center"] == centeredPlanet && centeredObject != "sun") || name == centeredObject);
			toggle = false;
		}
		else {
			planets[name].lensFlare.visible = false;
...
function shouldShow(name) { // Determine if it should show the planet based on the modifier arrays
	...
	if (planets[name]) {
		if (planets[name].viewingClass) {
			...
			if (planets[name].viewingClass.minorBody) { // If it is in a list, unless it is to be shown, don't show it
				changed = true;
				if (bodiesShow) {
					show = true;
				}
			}
			if (planets[name].viewingClass.expanse) { // ETC
				changed = true;
				if (expanseShow) {
					show = true;
				}
	...
	if (changed == false) { // If it's not in any list, show it anyway.
		show = true;
	}
...
function updateDropdowns() { // Update all the dropdowns when planets shown or hidden - and deal with if they are selected
	...
	for (var name in currentPositions) {
		if (shouldShow(name)) {
			if (name != "sun" && name != "ship") {
				if (planets[name]["center"] == EpsteinCenterValue.toLowerCase()) {
					printer += "<option value='" + name + "'>" + capitalise(name) + "</option>";
				}
			}
		}
		...
		if (shouldShow(name)) {
			if (name != "sun") {
				if (planets[name]["center"] == "sun") {
					var centerCount = 0;
					for (var planet in planets) {
						if (planets[planet]["center"] == name && shouldShow(planet)) {
							centerCount += 1;
						}
					}
					if (centerCount >= 2) {
	...
	document.getElementById("EpsteinCenter").innerHTML = "<option value='sun'>Sun</option>" + printer;
	document.getElementById("ILCenter").innerHTML = printer;
	...
	printer = "<option value='planet'>" + capitalise(centeredPlanet) + "</option>";
	for (var name in currentPositions) {
		if (shouldShow(name)) {
	...
	var centerCount = 0;
	for (var planet in planets) {
		if (planets[planet]["center"] == ILValue.toLowerCase() && shouldShow(planet)) {
			centerCount += 1;
		}
	}
	...
	var centerCount = 0;
	for (var planet in planets) {
		if (planets[planet]["center"] == EpsteinCenterValue.toLowerCase() && shouldShow(planet)) {
			centerCount += 1;
		}
	}
...
function startShipView() { // Initialize the ship camera view
...
function resetShipView() { // End the ship camera view, back to the normal orbital view
	cameraMode = "orbital";
...
function startShipCenter() { // Move the camera into centering around the ship
	cameraMode = "shipCenter";
}

function stopShipCenter() { // Move the camera back to normal viewing
...
function retractInstrumentPanels() { // Pull all panels back off the side
	document.getElementById("viewInstrumentPanel").style.left = "-250px";
	if (stereoDisp) {
		document.getElementById("backLink").style.right = (10 - document.getElementById("backLink").clientWidth / 4) + "px";
		document.getElementById("retractArrow").style.left = (0 - document.getElementById("retractArrow").clientWidth / 4) + "px";
		document.getElementById("retractDetails").style.right = (10 - document.getElementById("retractDetails").clientWidth / 4) + "px";
		document.getElementById("transferDetails").style.right = (10 - document.getElementById("transferDetails").clientWidth / 4) + "px";
	}
	else {
...
function extendInstrumentPanels() { // Push the panels into the viewframe
	if (stereoDisp) {
		document.getElementById("viewInstrumentPanel").style.left = (10 - document.getElementById("viewInstrumentPanel").clientWidth / 4) + "px";
		document.getElementById("backLink").style.right = (260 - document.getElementById("transferInstrumentPanel").clientWidth / 2 - document.getElementById("backLink").clientWidth / 4) + "px";
		document.getElementById("transferInstrumentPanel").style.right = (10 - document.getElementById("transferInstrumentPanel").clientWidth / 4) + "px";
		document.getElementById("retractArrow").style.left = (260 - document.getElementById("viewInstrumentPanel").clientWidth / 2 - document.getElementById("retractArrow").clientWidth / 4) + "px";

		document.getElementById("retractDetails").style.right = (260 - document.getElementById("viewInstrumentPanel").clientWidth / 2 - document.getElementById("retractDetails").clientWidth / 4) + "px";
		document.getElementById("transferDetails").style.right = (260 - document.getElementById("viewInstrumentPanel").clientWidth / 2 - document.getElementById("transferDetails").clientWidth / 4) + "px";
	}
	else {
...
function round(value, points) { // Round a value to a certain number of decimal points
...
function getObjectSize(obj) {
	...
	return magnitude(subVec(firstBound, secondBound)) / Math.sqrt(3) / scene.scale.x
}

// Transformation Functions

function M3S2toAU3Y2(value) { // Convert M3/S2 to AU3/Y2 - for gravParams
	return value * convertDistance("M", "AU", 3) * convertTime("S", "Y", -2);
}

function AU3Y2toM3S2(value) { // Convert AU3/Y2 to M3/S2 - for gravParams
	return value * convertDistance("AU", "M", 3) * convertTime("Y", "S", -2);
}

function convertTime(from, to, pow) { // Convert any time from a format to a format with a given power
	if (pow == undefined) {
		pow = 1;
	}
	var value = 1;
...
function convertDistance(from, to, pow) { // Convert any distance from anything to anything, and with a power modifier
...
function convertSpeed(from, to) { // Convert a speed in L/T format from anything to anything
	if (from == "C") {
		from = ["LY", "Y"]
	}
	else {
...
function DtoR(degrees) { // Degrees to Radians
	return (degrees / 180) * Math.PI;
}

function RtoD(radians) { // Radians to degrees
	return (radians / Math.PI) * 180;
}

function threeVector(array) { // Make a normal vector into a ThreeJS Vector3
	if (webVR) {
...
function reverseThreeVector(threeVector) { // Turn it from a threevector into an array-type vector
	if (webVR) {
...
function threeColourToRGB(threeColour) {
	return "rgb(" + round(threeColour.r * 255) + "," + round(threeColour.g * 255) + "," + round(threeColour.b * 255) + ")";
}

function calculateVectorFromRADE(RA, DE) { // Convert a Right Ascension and DEclination into 3-part vector
	...
	var vector = addVec(addVec(setMagnitude(VE, VEMag), setMagnitude(MI, MIMag)), setMagnitude(UP, UPMag))
...
function findPeriod(a, center) { // Find the period of a planet given the center and semi-major axis
	...
	return Math.pow(((4 * Math.pow(Math.PI, 2)) / gravitationalParameter) * Math.pow(a, 3), 1 / 2); // Period in years. 1AU = 1 year
}

function calculateSynodicPeriod(period1, period2) { // Calculate synodic period given two periods
...
function findGravParam(center) { // Find gravitational parameter of a body
	...
	if (center != "sun") { // If around a non-sun body, find the parameter
		if (planets[center]["mass"]) {
			gravitationalParameter = planets[center]["mass"] * gravitationalConstant;
		}
		else {
...
function dotProduct(vector1, vector2) { // Take the dot product of two vectors
	return (vector1[0] * vector2[0]) + (vector1[1] * vector2[1]) + (vector1[2] * vector2[2]);
}

function crossProduct(v1, v2) { // Return the cross product of two vectors
	return [v1[1] * v2[2] - v1[2] * v2[1], v1[2] * v2[0] - v1[0] * v2[2], v1[0] * v2[1] - v1[1] * v2[0]];
}

function multiplyVec(mul, vec) { // Multiply a vector by a number
	return [mul * vec[0], mul * vec[1], mul * vec[2]];
}

function addVec(vec1, vec2) { // Add vectors
	return [vec1[0] + vec2[0], vec1[1] + vec2[1], vec1[2] + vec2[2]];
}

function subVec(vec1, vec2) { // Subtract vectors
	return [vec1[0] - vec2[0], vec1[1] - vec2[1], vec1[2] - vec2[2]];
}

function magnitude(vector) { // Find the magnitude of a vector
	return Math.pow((vector[0] * vector[0]) + (vector[1] * vector[1]) + (vector[2] * vector[2]), 0.5);
}

function setMagnitude(vector, mag) { // Set the magnitude of a vector
	return multiplyVec(mag / magnitude(vector), vector);
}

function vectorToAngle(vector) { // Find what angle a given vector is at from the vernal equinox - ANTICLOCKWISE

	// Use tangents to find out what angle it is
	var degreesFromOne = ((RtoD(Math.atan(vector[1] / vector[0])))) % 360;

	if (vector[0] < 0) { // Compensate for arctan's blind spot
		degreesFromOne += 180;
	}
...
function angleBetweenVectors(vec1, vec2) { // Find the anticlockwise angle between vectors
	...
	var angle = RtoD(Math.acos(dotProduct(vec1, vec2) / (magnitude(vec1) * magnitude(vec2))));
...
function capitalise(string) { // Capitalise the given string
...
function checkEasterEggs() { // This is called when disabling the SOI sphere

	if (eggEnabled == false) { // Check if it should activate the easter eggs
		if (centeredObject == "iapetus") {
			if (ASZPlayed == false) { // If it's looking at iapetus, play ASZ if it hasn't already
				ASZPlayed == true;
				playSound("ASZ");
			...
			if (document.getElementById("ASZ").currentTime > 8.177516) { // Time determined experimentally - stop if after that
				swal("You found the Easter Egg!", "You have found the Monolith. Don't worry, this program won't turn homicidal (we hope)", "success");
				egg();
		...
		if (centeredObject == "eros") { // Activate upon reaching Eros
			swal("You found the Easter Egg!", "Welcome to Eros Station! Everyone's a winner on Eros...", "success");
			egg();
		}
		if (centeredObject == "phoebe") { // Activate upon reaching Phoebe
			swal("You found the Easter Egg!", "Welcome to Phoebe Station. Watch out for alien superweapons and MCRN missiles.", "success");
			egg();
		}
...
function egg() { // Enable the easter egg functions
	if (eggEnabled == false) {
...
function playSound(key) { // Play a given sound
	var sound = document.getElementById(key);
	sound.play();
	setTimeout(function () { // Stop it after 20 seconds
		sound.pause();
	}, 20000);
}
...
function launchNauvoo() { // Begin Nauvoo Launch
	...
	// Get the Nauvoo mesh
	nauvooMeshStor = planets["tycho station"].surfaceMesh.children[0].children[1];
...
function updateNauvoo() {
	...
	// Find time
	var time = (new Date().getTime() - launchTimer.getTime()) * convertTime("MS", "S");
	...
	if (time >= 0 && time <= 5) { // Clamps disengaged

		// Update caption
		document.getElementById('caption').innerHTML = "Docking clamps disengaged"
		...
		var pos = (finalPos - initialPos) * (remainingTime / sectionTime) + initialPos;

		// Set the current position
		planets["tycho station"].surfaceMesh.children[0].children[0].children[3].position.y = pos

		// Set effective current angle
		currentAngle = 0;

		// Set position and rotation of the Nauvoo to a constant
		nauvooMesh.position.set(0, -(250 + 400) * convertDistance("M", "AU") * totalScale, 0);
		var axis = threeVector([-1, 0, 0])
		nauvooMeshStor.quaternion.setFromUnitVectors(axis, threeVector([-Math.cos(currentAngle), 0, Math.sin(currentAngle) * directionControl]))
		nauvooMesh.visible = true;

	}
	else if (time >= 5 && time <= 15) { // Engaging reorientation of the Nauvoo

		// Update caption
		document.getElementById('caption').innerHTML = "Commencing reorientation of the Nauvoo";
		...
		if ((remainingTime / sectionTime) < (1 / 2)) { // If in the first phase, it's accelerating
			currentAngle = 1 / 2 * angAccel * Math.pow(remainingTime, 2);
		}
		else { // If in the second phase, it's decelerating
			currentAngle = finalAngle - (1 / 2 * angAccel * Math.pow(sectionTime - remainingTime, 2));
		}

		// Set initial position of the Nauvoo
		initialPos = [0, -(250 + 400) * convertDistance("M", "AU") * totalScale, 0];
		...
		var relativePos = [-Math.sin(currentAngle), 0, -Math.cos(currentAngle) * directionControl];
		...
		nauvooMeshStor.quaternion.setFromUnitVectors(threeVector([-1, 0, 0]), threeVector([-Math.cos(currentAngle), 0, Math.sin(currentAngle) * directionControl]))
	}
	else if (time >= 15 && time <= 23) { // Nauvoo set for optimal trajectory
		...
		// Update the caption
		if (remainingTime > 5) {
			document.getElementById('caption').innerHTML = "Main engine powerup in T-" + Math.ceil(sectionTime - remainingTime) + " seconds"
		}
		else {
			document.getElementById('caption').innerHTML = "Nauvoo is now set for optimal trajectory"
		}
		...
		relativePos = [-Math.sin(currentAngle), 0, -Math.cos(currentAngle) * directionControl];
		relativePos = setMagnitude(relativePos, radius);
	...
	else if (time >= 23 && time <= 38) { // Launch the Nauvoo

		// Update the caption
		document.getElementById('caption').innerHTML = "Good luck, and Godspeed"
		...
		var direction = [-Math.cos(currentAngle), 0, Math.sin(currentAngle) * directionControl];
		...
		else if (remainingTime > sectionTime - 10 && remainingTime < sectionTime - 8) {
			document.getElementById('caption').innerHTML = "Guys, what just happened?"
		}
		else if (remainingTime > sectionTime - 8 && remainingTime < sectionTime - 6) {
			document.getElementById('caption').innerHTML = "Guys, what just happened?<br><br>The Nauvoo just missed!"
		}
		else if (remainingTime > sectionTime - 6 && remainingTime < sectionTime - 4) {
			document.getElementById('caption').innerHTML = ""
		}
		else if (remainingTime > sectionTime - 4 && remainingTime < sectionTime - 2) {
			document.getElementById('caption').innerHTML = "The Nauvoo didn't move"
		}
		else {
			document.getElementById('caption').innerHTML = "The Nauvoo didn't move<br><br>Eros did";
		}
		...
		if (remainingTime >= (sectionTime / 2) - accelTime && remainingTime <= sectionTime / 2) { // Eros is accelerating away
			var distance = 0.5 * accel * Math.pow(remainingTime - ((sectionTime / 2) - accelTime), 2)
			deflectPos = setMagnitude(finalDeflectPos, distance)
		}
		else if (remainingTime >= (sectionTime / 2) && remainingTime <= (sectionTime / 2) + accelTime) { // Eros is decelerating to a relative stop
			var distance = totalDeflect - (0.5 * accel * Math.pow(remainingTime - ((sectionTime / 2) + accelTime), 2))
			deflectPos = setMagnitude(finalDeflectPos, distance)
		}
		else if (remainingTime > (sectionTime / 2) + accelTime) { // Eros is at a relative stop at the total defelction
		...
		var nauvooPos = addVec(erosPos, addVec(initialPos, addVec(deflectPos, posDiff)));

		// Set the position of the Nauvoo
		nauvooMesh.position.copy(threeVector(nauvooPos));
	}
	else { // Finalise launch processes
	...
	if (time >= 0 && time <= 35) { // Viewing Tycho

		// Derive the components of the camera position (these are pre-set)
		var cameraVerticalPoint = multiplyVec(convertDistance("M", "AU") * totalScale, [0, 0, 1000]);
		var cameraHorizontalPoint = setMagnitude(currentPositions["tycho station"], -6000 * convertDistance("M", "AU") * totalScale);
		...
		var centralPoint = multiplyVec(totalScale, currentPositions["tycho station"]);

		// Set the camera position
		camera.position.copy(threeVector(addVec(cameraPoint, centralPoint)));
		controls.update();
	}
	if (time >= 38 && time <= 53) { // Viewing Eros
		...
		// Derive the components of the camera position (these are pre-set)
		cameraVerticalPoint = multiplyVec(convertDistance("M", "AU") * totalScale, [0, 0, 60000]);
		cameraHorizontalPoint = setMagnitude(currentPositions["eros"], -60000 * convertDistance("M", "AU") * totalScale);
...
function startSystemTour() { // Start the Solar System tour
...
function prepareSystemTour() { // Prepare the system graphics for the tour

	for (var name in tourStops) { // Iterate through each object in the tour

		if (name != "sun") { // Except the sun (because it's special)

			// Load the model if it has one, otherwise load the surface
			if (planets[centeredObject]["mapClass"]["model"]) {
				if (planets[centeredObject]["surfaceMesh"].geometry.type == "SphereGeometry") {
					loadModel(centeredObject);
				}
			}
			else {
...
function calculateSystemTour() { // Calculate all the actions in the tour
	...
	var movements = [{
		...
		end: addVec(setMagnitude([0, -1, 0], tourStops[stops[0]]), currentPositions[stops[0]]),
		type: "translation",
		firstCamera: "sun",
	...
	for (var stopNum = 0; stopNum < stops.length - 1; stopNum++) {
		...
		var transitVector = subVec(currentPositions[stops[stopNum + 1]], currentPositions[stops[stopNum]])
		var standOffPos = setMagnitude(transitVector, tourStops[stops[stopNum]])
		var finalPosition = multiplyVec(-1, standOffPos)

		// Create the rotation
		var rotation = {
			start: subVec(movements[movements.length - 1].end, currentPositions[stops[stopNum]]),
			end: finalPosition,
			type: "rotation",
		...
		var ratio = dotProduct(rotation.start, rotation.end) / (magnitude(rotation.start) * magnitude(rotation.end))
		...
		var auxAxis = setMagnitude(crossProduct(rotationAxis, rotation.start), tourStops[rotation.center])

		// Find the two position components
		var firstModeratedPos = multiplyVec(Math.cos(DtoR(currentRotation)), rotation.start)
		var secondModeratedPos = multiplyVec(Math.sin(DtoR(currentRotation)), auxAxis)

		// Find the final rotation position and the starting translation position
		var startPosition = setMagnitude(addVec(firstModeratedPos, secondModeratedPos), tourStops[rotation.center])

		// Negative one because it points from the destination TO the origin planet
		var finalStandOff = setMagnitude(transitVector, -1 * tourStops[stops[stopNum + 1]])
		var finalArrivePosition = addVec(finalStandOff, currentPositions[stops[stopNum + 1]])
	...
	var rotation = {
		start: subVec(movements[movements.length - 1].end, currentPositions[stops[stops.length - 1]]),
	...
	var translation = {
		start: addVec(movements[movements.length - 1].end, currentPositions[stops[stops.length - 1]]),
	...
	for (var index in movements) {
		...
		if (movement.type == "translation") {
			...
			if (distance > 2 * (1 / 2) * tourTranslationAcceleration * Math.pow(tourTranslationSpeed / tourTranslationAcceleration, 2)) {
				// Calculate the non-acclerating distance
				var remainingDist = distance - 2 * (1 / 2) * tourTranslationAcceleration * Math.pow(tourTranslationSpeed / tourTranslationAcceleration, 2)
				...
				accelTime = Math.pow(distance / tourTranslationAcceleration, 1 / 2)
			...
			movements[index].accelDist = (1 / 2) * tourTranslationAcceleration * Math.pow(accelTime, 2)
			movements[index].coastDist = coastTime * tourTranslationSpeed;
			movements[index].decelDist = (1 / 2) * tourTranslationAcceleration * Math.pow(decelTime, 2)

			// Calculate the total distance it moves
			movements[index].totalDist = movements[index].accelDist + movements[index].coastDist + movements[index].decelDist
			...
			var ratio = dotProduct(movement.start, movement.end) / (magnitude(movement.start) * magnitude(movement.end))
			var angle = RtoD(Math.acos(ratio));
			...
			if (movement.auxType == "overshoot") { // If it overshoots to look at the next planet, add the additional degrees
...
function updateSystemTour() {
	...
	while (selectedTime + movement.movementTime < time && index < tourMovements.length) {
	...
	if (time > tourMovements[tourMovements.length - 1].startTime + tourMovements[tourMovements.length - 1].movementTime) {
		endSystemTour();
	}
	else {
		...
		var percentThrough = (time - movement.startTime) / movement.movementTime;
		...
		if (movement.type == "rotation") {
			...
			if (currentRotation > movement.cameraAngle) {
				...
				message = movement.messageObject + "<br>Next up: " + movement.secondCamera;
			}
			else {
			...
			var auxAxis = setMagnitude(crossProduct(rotationAxis, movement.start), tourStops[movement.center])

			// Find the two sections of the position
			var firstModeratedPos = multiplyVec(Math.cos(DtoR(currentRotation)), movement.start)
			var secondModeratedPos = multiplyVec(Math.sin(DtoR(currentRotation)), auxAxis)

			// Set the position
			position = addVec(setMagnitude(addVec(firstModeratedPos, secondModeratedPos), tourStops[movement.center]), currentPositions[movement.center])
		}
		else if (movement.type == "translation") { // If moving between planets

			// Find the distance it's moved through the transfer
			var distance;
			if (elapsedTime < movement.accelTime) { // In acceleration phase
				distance = 0;
				distance += (1 / 2) * tourTranslationAcceleration * Math.pow(elapsedTime, 2)
			}
			else if (elapsedTime < movement.accelTime + movement.coastTime) { // In coast phase
				distance = movement.accelDist;
				distance += tourTranslationSpeed * (elapsedTime - movement.accelTime)
			}
			else if (elapsedTime < movement.movementTime) { // In deceleration phase
				var momentTime = (elapsedTime - movement.accelTime - movement.coastTime)

				distance = movement.accelDist + movement.coastDist;
				distance += (movement.accelTime * tourTranslationAcceleration) * momentTime - ((1 / 2) * tourTranslationAcceleration * Math.pow(momentTime, 2))
			}

			// Find the current position
			percentThrough = distance / movement.totalDist
			position = addVec(multiplyVec(percentThrough, movement.end), multiplyVec(1 - percentThrough, movement.start))
			position = addVec(movement.start, setMagnitude(subVec(movement.end, movement.start), distance))

			// Manage the camera target and how it moves
			if (movement.cameraMove == "slide") {
				target = addVec(multiplyVec(1 - percentThrough, currentPositions[movement.firstCamera]), multiplyVec(percentThrough, currentPositions[movement.secondCamera]))
			}
			...
			if (movement.secondCamera != centeredObject) {
				...
				changeCenter();
				if (planets[movement.secondCamera].center != "sun") {
					document.getElementById("moonSelect").value = movement.secondCamera
					changeMoon();
				}
			}
...
var kerbalMessages = [ // Kerbal style loading messages
...
var realMessages = [ // More serious messages that actually say what's going on
...
var now = new Date();
var displayTime = now;
var currentTime = new Date(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), now.getUTCHours(), now.getUTCMinutes(), now.getUTCSeconds()); // May not actually be now, just when it is displayed
...
var marsOpacity = Math.random() * (marsOpacityUpper - marsOpacityLower) + marsOpacityLower;
