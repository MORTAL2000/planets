...
void PlanetCamera::update(double deltaTime, Planet *plttt)
{
    ...
    cam->position = mu::Y * glm::vec3(5. + 235 * (1. - actualZoom));
    ...
    cam->update();
    if (prevPos != cam->position) planetCulling();
}
...
void PlanetCamera::dragUpdate()
{
    ...
    if (plt->rayToLonLat(cam->position, rayDir, cursorLonLat))
    {
